//! Scalar-level MSL translation helpers.
//!
//! Pure functions that map IR enum variants to MSL syntax fragments.

use quanta_ir::*;

pub(crate) fn const_msl(v: &ConstValue) -> (&'static str, String) {
    match v {
...
pub(crate) fn math_fn_str(f: &MathFn) -> &'static str {
    match f {
        MathFn::Sin => "sin",
        MathFn::Cos => "cos",
        MathFn::Tan => "tan",
...
pub(crate) fn atomic_fn_str(op: &AtomicOp) -> &'static str {
    match op {
