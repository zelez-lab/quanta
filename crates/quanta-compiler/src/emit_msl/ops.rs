...
pub(crate) fn emit_op(
    ...
    match op {
        ...
        KernelOp::TextureSample2D {
            ...
            out.push_str(&format!(
                "{}{} = tex_{}.sample(samp_{}, float2(r{}, r{})).x;\n",
                pad,
                dst_lv(mutable, ty.msl_name(), dst.0),
                texture,
                texture,
        ...
        KernelOp::TextureSample3D {
            ...
            out.push_str(&format!(
                "{}{} = tex_{}.sample(samp_{}, float3(r{}, r{}, r{})).x;\n",
                pad,
                dst_lv(mutable, ty.msl_name(), dst.0),
                texture,
                texture,
        ...
        KernelOp::CooperativeMMA {
            ...
            // existing fragment in place — re-declaring it would shadow the
            // accumulator declared outside the loop (and read it uninitialised).
            // Otherwise declare a fresh result matrix.
            let t = simd_elem(ty);
            if dst.0 == c.0 {
                out.push_str(&format!(
...
fn simd_elem(ty: &ScalarType) -> &'static str {
    match ty {
        ScalarType::F16 => "half",
        _ => "float",
    }
