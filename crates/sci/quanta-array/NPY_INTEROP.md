...
| ZIP64 (≥ 4 GiB archives / entries, > 65 535 entries) | **deferred (loud error, claim boundary)** | The reader's CLAIM is "reads what numpy writes at the scales this stack exchanges" (ratification record #4): numpy emits ZIP64 SIZES only above 4 GiB, and safetensors is the declared big-weights lane. Archive-level ZIP64 (CD extras, the EOCD locator, real ≥4 GiB sizes) is detected and refused loudly rather than misparsed. NUANCE, taught by the real fixtures: CPython's zipfile streams LOCAL headers with `0xFFFFFFFF` size sentinels + a local ZIP64 extra at ANY size — every real numpy .npz carries them — so local sentinels defer to the authoritative CD instead of refusing. |
