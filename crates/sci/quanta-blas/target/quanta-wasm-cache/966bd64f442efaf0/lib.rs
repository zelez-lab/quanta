...
chol_col_upper_f32(a : * mut f32, n : u32, j : u32, z : u32, s : u32)
{
    ...
    {
        {
            ...
            = * (a.add((((j * n + i) ^ z) as usize) as usize)); let mut p :
            u32 = 0u32; while p < steps
            {
                let uqj = * (a.add((((p * n + j) ^ z) as usize) as usize));
                let uqi = * (a.add((((p * n + i) ^ z) as usize) as usize));
                acc = acc - uqj * uqi; p = p + s;
            } if active == 1u32
            { * (a.add((((j * n + i) ^ z) as usize) as usize)) = acc / ujj; }
        }
    }
}
