...
chol_col_lower_f32(a : * mut f32, n : u32, j : u32, z : u32, s : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let active = if i < n
            { if i > j { 1u32 } else { 0u32 } } else { 0u32 }; let steps =
            active * j; let ljj = *
            (a.add((((j * n + j) ^ z) as usize) as usize)); let mut acc : f32
            = * (a.add((((i * n + j) ^ z) as usize) as usize)); let mut p :
            u32 = 0u32; while p < steps
            {
                let lip = * (a.add((((i * n + p) ^ z) as usize) as usize));
                let ljp = * (a.add((((j * n + p) ^ z) as usize) as usize));
                acc = acc - lip * ljp; p = p + s;
            } if active == 1u32
            { * (a.add((((i * n + j) ^ z) as usize) as usize)) = acc / ljj; }
        }
    }
}
