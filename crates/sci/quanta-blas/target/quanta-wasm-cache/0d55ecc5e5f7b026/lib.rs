...
syr2k_f32(a : * const f32, b : * const f32, c : * mut f32, n : u32, k : u32,
rs : u32, cs : u32, lower : u32, alpha : f32, beta : f32, z : u32, s : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let gid = quark_id(); let total = n * n; let idx = if gid < total
            { gid } else { 0u32 }; let row = idx / n; let col = idx - row * n;
            let le = if col <= row { 1u32 } else { 0u32 }; let ge = if row <=
            col { 1u32 } else { 0u32 }; let tri = if lower == 1u32 { le } else
            { ge }; let inb = if gid < total { 1u32 } else { 0u32 }; let mut
            acc : f32 = 0.0f32; let mut p : u32 = 0u32; while p < k
            {
                let ai = *
                (a.add((((row * rs + p * cs) ^ z) as usize) as usize)); let bj
                = * (b.add((((col * rs + p * cs) ^ z) as usize) as usize));
                acc = acc + ai * bj; p = p + s;
            } let mut acc2 : f32 = 0.0f32; let mut q : u32 = 0u32; while q < k
            {
                let bi = *
                (b.add((((row * rs + q * cs) ^ z) as usize) as usize)); let aj
                = * (a.add((((col * rs + q * cs) ^ z) as usize) as usize));
                acc2 = acc2 + bi * aj; q = q + s;
            } let acc = acc + acc2; let cv = *
            (c.add((idx as usize) as usize)); let result = alpha * acc + beta
            * cv; let ok = inb * tri; if ok == 1u32
