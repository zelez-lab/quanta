...
qr_qt_apply_f32(b : * mut f32, v : * const f32, tau : * const f32, m : u32, n
: u32, nrhs : u32, k : u32, z : u32, s : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let j = quark_id(); let active = if j < nrhs { 1u32 } else
            { 0u32 }; let tau_k = * (tau.add(((k ^ z) as usize) as usize));
            let mut dot : f32 = 0.0f32; let mut p : u32 = active * k; let plim
            : u32 = active * m; while p < plim
            {
                let vp = * (v.add((((p * n + k) ^ z) as usize) as usize)); let
                bpj = * (b.add((((p * nrhs + j) ^ z) as usize) as usize)); dot
                = dot + vp * bpj; p = p + s;
            } let scale = tau_k * dot; let mut i : u32 = active * k; let ilim
            : u32 = active * m; while i < ilim
            {
                let vi = * (v.add((((i * n + k) ^ z) as usize) as usize)); let
                bij = * (b.add((((i * nrhs + j) ^ z) as usize) as usize)); let
                upd = bij - scale * vi; if active == 1u32
                { * (b.add((((i * nrhs + j) ^ z) as usize) as usize)) = upd; }
                i = i + s;
            }
        }
    }
