...
lu_swap_rows_f32(a : * mut f32, n : u32, k : u32, r : u32, z : u32, s : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let j = quark_id(); let active = if j < n { 1u32 } else { 0u32 };
            let col = if active == 1u32 { j } else { 0u32 }; let vk = *
            (a.add((((k * n + col) ^ z) as usize) as usize)); let vr = *
            (a.add((((r * n + col) ^ z) as usize) as usize)); let one = if s <
            2u32 { 1u32 } else { 1u32 }; if active == one
            {
                * (a.add((((k * n + col) ^ z) as usize) as usize)) = vr; *
                (a.add((((r * n + col) ^ z) as usize) as usize)) = vk;
            }
        }
    }
}
