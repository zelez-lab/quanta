...
jacobi_rot_cols_f32(a : * mut f32, n : u32, p : u32, q : u32, c : f32, s :
f32, z : u32, s_step : u32)
{
    ...
    {
        {
            ...
            < steps
            {
                let akp = * (a.add((((k * n + p) ^ z) as usize) as usize));
                let akq = * (a.add((((k * n + q) ^ z) as usize) as usize)); *
                (a.add((((k * n + p) ^ z) as usize) as usize)) = c * akp - s *
                akq; * (a.add((((k * n + q) ^ z) as usize) as usize)) = s *
                akp + c * akq; done = done + s_step;
            }
        }
    }
}
