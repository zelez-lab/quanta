...
qr_house_f32(a : * mut f32, v : * mut f32, tau : * mut f32, m : u32, n : u32,
k : u32, z : u32, s : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let lane = quark_id(); let active = if lane < 1u32 { 1u32 } else
            { 0u32 }; let mut nrm2 : f32 = 0.0f32; let mut i : u32 = active *
            k; let ilim : u32 = active * m; while i < ilim
            {
                let xi = * (a.add((((i * n + k) ^ z) as usize) as usize));
                nrm2 = nrm2 + xi * xi; i = i + s;
            } let xnorm = sqrt(nrm2); let xk = *
            (a.add((((k * n + k) ^ z) as usize) as usize)); let sgn = if xk <
            0.0f32 { - 1.0f32 } else { 1.0f32 }; let alpha = - sgn * xnorm;
            let vk = xk - alpha; let tail = nrm2 - xk * xk; let vtv = vk * vk
            + tail; let tau_k = if vtv > 0.0f32 { 2.0f32 / vtv } else
            { 0.0f32 }; let mut w : u32 = active * k; let wlim : u32 = active
            * m; while w < wlim
            {
                let xw = * (a.add((((w * n + k) ^ z) as usize) as usize)); let
                vw = if w == k { vk } else { xw }; *
                (v.add((((w * n + k) ^ z) as usize) as usize)) = vw; if w > k
                { * (a.add((((w * n + k) ^ z) as usize) as usize)) = vw; } w =
                w + s;
            } if active == 1u32
            {
                * (a.add((((k * n + k) ^ z) as usize) as usize)) = alpha; *
                (tau.add(((k ^ z) as usize) as usize)) = tau_k;
            }
        }
    }
}
