...
lu_elim_f32(a : * mut f32, n : u32, k : u32, z : u32, s : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let active = if i < n
            { if i > k { 1u32 } else { 0u32 } } else { 0u32 }; let akk = *
            (a.add((((k * n + k) ^ z) as usize) as usize)); let aik = *
            (a.add((((i * n + k) ^ z) as usize) as usize)); let m = aik / akk;
            if active == 1u32
            { * (a.add((((i * n + k) ^ z) as usize) as usize)) = m; } let span
            = if active == 1u32 { n } else { 0u32 }; let mut j : u32 = k +
            1u32; while j < span
            {
                let akj = * (a.add((((k * n + j) ^ z) as usize) as usize));
                let aij = * (a.add((((i * n + j) ^ z) as usize) as usize)); *
                (a.add((((i * n + j) ^ z) as usize) as usize)) = aij - m *
                akj; j = j + s;
            }
        }
    }
