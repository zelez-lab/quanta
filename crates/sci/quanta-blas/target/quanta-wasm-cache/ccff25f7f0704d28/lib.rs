...
jacobi_col_rot_f32(mat : * mut f32, rows : u32, cols : u32, p : u32, q : u32,
c : f32, s : f32, z : u32, s_step : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let active = if i < rows { 1u32 } else
            { 0u32 }; let steps = active * s_step; let mut done : u32 = 0u32;
            while done < steps
            {
                let mip = *
                (mat.add((((i * cols + p) ^ z) as usize) as usize)); let miq =
                * (mat.add((((i * cols + q) ^ z) as usize) as usize)); *
                (mat.add((((i * cols + p) ^ z) as usize) as usize)) = c * mip
                - s * miq; *
                (mat.add((((i * cols + q) ^ z) as usize) as usize)) = s * mip
                + c * miq; done = done + s_step;
            }
        }
    }
}
