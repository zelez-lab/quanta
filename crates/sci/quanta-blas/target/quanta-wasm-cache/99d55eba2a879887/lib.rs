...
chol_diag_lower_f32(a : * mut f32, n : u32, j : u32, z : u32, s : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let lane = quark_id(); let active = if lane < 1u32 { 1u32 } else
            { 0u32 }; let steps = active * j; let mut d : f32 = *
            (a.add((((j * n + j) ^ z) as usize) as usize)); let mut p : u32 =
            0u32; while p < steps
            {
                let ljp = * (a.add((((j * n + p) ^ z) as usize) as usize)); d
                = d - ljp * ljp; p = p + s;
            } if active == 1u32
            { * (a.add((((j * n + j) ^ z) as usize) as usize)) = sqrt(d); }
        }
    }
}
