...
gemm_f32_naive(a : * const f32, b : * const f32, c : * mut f32, m : u32, n :
u32, k : u32, alpha : f32, beta : f32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let total = m * n; let idx = if i < total
            { i } else { 0u32 }; let row = idx / n; let col = idx % n; let mut
            acc : f32 = 0.0f32; let mut p : u32 = 0u32; while p < k
            {
                let av = * (a.add(((row * k + p) as usize) as usize)); let bv
                = * (b.add(((p * n + col) as usize) as usize)); acc = acc + av
                * bv; p = p + 1u32;
            } let cv = * (c.add((idx as usize) as usize)); let result = alpha
            * acc + beta * cv; if i < total
