...
trmm_lower_f32(a : * const f32, b : * mut f32, nt : u32, nlanes : u32, lb :
u32, ts : u32, rs : u32, cs : u32, unit : u32, alpha : f32, z : u32, s : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let lane = quark_id(); let base = lane * lb; let rows = if lane <
            nlanes { nt } else { 0u32 }; let mut t : u32 = 0u32; while t <
            rows
            {
                let i = rows - 1u32 - t; let mut acc : f32 = 0.0f32; let mut p
                : u32 = 0u32; while p <= i
                {
                    let mip = *
                    (a.add((((i * rs + p * cs) ^ z) as usize) as usize)); let
                    on_diag = if p == i { 1u32 } else { 0u32 }; let m_eff = if
                    (unit == 1u32) & (on_diag == 1u32) { 1.0f32 } else { mip };
                    let bp = *
                    (b.add((((base + p * ts) ^ z) as usize) as usize)); acc =
                    acc + m_eff * bp; p = p + s;
                } * (b.add((((base + i * ts) ^ z) as usize) as usize)) = alpha
                * acc; t = t + 1u32;
            }
        }
    }
