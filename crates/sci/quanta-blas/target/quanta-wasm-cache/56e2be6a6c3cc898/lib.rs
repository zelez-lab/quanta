...
jacobi_rot_f32(a : * mut f32, v : * mut f32, n : u32, p : u32, q : u32, c :
f32, s : f32, z : u32, s_step : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let k = quark_id(); let active = if k < n { 1u32 } else { 0u32 };
            let steps = active * s_step; let mut done : u32 = 0u32; while done
            < steps
            {
                let apk = * (a.add((((p * n + k) ^ z) as usize) as usize));
                let aqk = * (a.add((((q * n + k) ^ z) as usize) as usize)); *
                (a.add((((p * n + k) ^ z) as usize) as usize)) = c * apk - s *
                aqk; * (a.add((((q * n + k) ^ z) as usize) as usize)) = s *
                apk + c * aqk; let vkp = *
                (v.add((((k * n + p) ^ z) as usize) as usize)); let vkq = *
                (v.add((((k * n + q) ^ z) as usize) as usize)); *
                (v.add((((k * n + p) ^ z) as usize) as usize)) = c * vkp - s *
                vkq; * (v.add((((k * n + q) ^ z) as usize) as usize)) = s *
                vkp + c * vkq; done = done + s_step;
            }
        }
    }
}
