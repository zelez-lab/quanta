...
syrk_f32(a : * const f32, c : * mut f32, n : u32, k : u32, ars : u32, acs :
u32, lower : u32, alpha : f32, beta : f32, z : u32, s : u32)
{
    ...
    {
        {
            ...
            {
                let av = *
                (a.add((((row * ars + p * acs) ^ z) as usize) as usize)); let
                bv = *
                (a.add((((col * ars + p * acs) ^ z) as usize) as usize)); acc
                = acc + av * bv; p = p + s;
            } let cv = * (c.add((idx as usize) as usize)); let result = alpha
            * acc + beta * cv; let ok = inb * tri; if ok == 1u32
