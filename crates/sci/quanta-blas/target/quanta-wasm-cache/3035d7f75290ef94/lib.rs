...
gemm_f32_tiled(a : * const f32, b : * const f32, c : * mut f32, m : u32, n :
u32, k : u32, alpha : f32, beta : f32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let gid = quark_id(); let block = gid / 256u32; let tid = gid %
            256u32; let tr = tid / 16u32; let tc = tid % 16u32; let local = tr
            * 16u32 + tc; let tiles_n = (n + 15u32) / 16u32; let block_row =
            block / tiles_n; let block_col = block % tiles_n; let row =
            block_row * 16u32 + tr; let col = block_col * 16u32 + tc; let
            row_ok = if row < m { 1.0f32 } else { 0.0f32 }; let col_ok = if
            col < n { 1.0f32 } else { 0.0f32 }; let row_c = if row < m { row }
            else { 0u32 }; let col_c = if col < n { col } else { 0u32 }; let
            num_k_tiles = (k + 15u32) / 16u32; let mut acc : f32 = 0.0f32; let
            mut kt : u32 = 0u32; while kt < num_k_tiles
            {
                let a_col = kt * 16u32 + tc; let a_kmask = if a_col < k
                { 1.0f32 } else { 0.0f32 }; let a_col_c = if a_col < k
                { a_col } else { 0u32 }; let a_val = *
                (a.add(((row_c * k + a_col_c) as usize) as usize)) *
                (row_ok * a_kmask);
                shared_store_f32(0u32, (local as usize) as u32, a_val); let
                b_row = kt * 16u32 + tr; let b_kmask = if b_row < k { 1.0f32 }
                else { 0.0f32 }; let b_row_c = if b_row < k { b_row } else
                { 0u32 }; let b_val = *
                (b.add(((b_row_c * n + col_c) as usize) as usize)) *
                (b_kmask * col_ok);
                shared_store_f32(1u32, (local as usize) as u32, b_val); unsafe
                { barrier() }; let mut p : u32 = 0u32; while p < 16u32
                {
                    let av =
                    shared_load_f32(0u32, ((tr * 16u32 + p) as usize) as u32);
                    let bv =
                    shared_load_f32(1u32, ((p * 16u32 + tc) as usize) as u32);
                    acc = acc + av * bv; p = p + 1u32;
                } unsafe { barrier() }; kt = kt + 1u32;
            } let in_bounds = row_ok * col_ok; let cv = *
            (c.add(((row * n + col) as usize) as usize)); let result = alpha *
            acc + beta * cv; if in_bounds > 0.5f32
            { * (c.add(((row * n + col) as usize) as usize)) = result; }
        }
    }
}
