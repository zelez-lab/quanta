...
qr_apply_f32(a : * mut f32, v : * const f32, tau : * const f32, m : u32, n :
u32, k : u32, z : u32, s : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let j = quark_id(); let active = if j < n
            { if j > k { 1u32 } else { 0u32 } } else { 0u32 }; let tau_k = *
            (tau.add(((k ^ z) as usize) as usize)); let mut dot : f32 =
            0.0f32; let mut p : u32 = active * k; let plim : u32 = active * m;
            while p < plim
            {
                let vp = * (v.add((((p * n + k) ^ z) as usize) as usize)); let
                apj = * (a.add((((p * n + j) ^ z) as usize) as usize)); dot =
                dot + vp * apj; p = p + s;
            } let scale = tau_k * dot; let mut i : u32 = active * k; let ilim
            : u32 = active * m; while i < ilim
            {
                let vi = * (v.add((((i * n + k) ^ z) as usize) as usize)); let
                aij = * (a.add((((i * n + j) ^ z) as usize) as usize)); let
                upd = aij - scale * vi; if active == 1u32
                { * (a.add((((i * n + j) ^ z) as usize) as usize)) = upd; } i
                = i + s;
            }
        }
    }
