...
trsm_bwd_f32(a : * const f32, x : * mut f32, nt : u32, nlanes : u32, lb : u32,
ts : u32, rs : u32, cs : u32, alpha : f32, z : u32, s : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let lane = quark_id(); let base = lane * lb; let rows = if lane <
            nlanes { nt } else { 0u32 }; let mut ii : u32 = 0u32; while ii <
            rows
            {
                let i = (nt - 1u32) - ii; let mut acc : f32 = alpha * *
                (x.add((((base + i * ts) ^ z) as usize) as usize)); let mut p
                : u32 = i + 1u32; while p < nt
                {
                    let av = *
                    (a.add((((i * rs + p * cs) ^ z) as usize) as usize)); let xv
                    = * (x.add((((base + p * ts) ^ z) as usize) as usize)); acc
                    = acc - av * xv; p = p + s;
                } let d = *
                (a.add((((i * (rs + cs)) ^ z) as usize) as usize)); *
                (x.add((((base + i * ts) ^ z) as usize) as usize)) = acc / d;
                ii = ii + 1u32;
            }
        }
    }
