...
symm_f32(a : * const f32, bmat : * const f32, c : * mut f32, m : u32, n : u32,
d : u32, sidel : u32, lower : u32, alpha : f32, beta : f32, z : u32, s : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let gid = quark_id(); let total = m * n; let idx = if gid < total
            { gid } else { 0u32 }; let row = idx / n; let col = idx - row * n;
            let inb = if gid < total { 1u32 } else { 0u32 }; let mut acc : f32
            = 0.0f32; let mut p : u32 = 0u32; while p < d
            {
                let sr = if sidel == 1u32 { row } else { p }; let sc = if
                sidel == 1u32 { p } else { col }; let le = if sc <= sr
                { 1u32 } else { 0u32 }; let ge = if sr <= sc { 1u32 } else
                { 0u32 }; let in_tri = if lower == 1u32 { le } else { ge };
                let a_in = sr * d + sc; let a_sw = sc * d + sr; let a_idx = if
                in_tri == 1u32 { a_in } else { a_sw }; let av = *
                (a.add(((a_idx ^ z) as usize) as usize)); let b_idx = if sidel
                == 1u32 { p * n + col } else { row * d + p }; let bv = *
                (bmat.add(((b_idx ^ z) as usize) as usize)); acc = acc + av *
                bv; p = p + s;
            } let cv = * (c.add((idx as usize) as usize)); let result = alpha
            * acc + beta * cv; if inb == 1u32
            { * (c.add((idx as usize) as usize)) = result; }
        }
    }
}
