...
fill_poisson_u32(out : * mut u32, lambda : f32, seed_hi : u32, seed_lo : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let id = quark_id(); let lam : f32 = lambda; let l_threshold : f32
            = exp(0.0f32 - lam); let mut p : f32 = 1.0f32; let mut k : u32 =
            0u32; let mut iter : u32 = 0u32; while iter < 64u32
            {
                let r : u32 =
                philox4x32_10_first_u32_kernel(id, iter, 0u32, 0u32, seed_lo,
                seed_hi); let bits : u32 = r >> 8u32; let u : f32 =
                (bits as f32) * (1.0f32 / 16_777_216.0f32); p = p * u; if p <=
                l_threshold { break; } k = k + 1u32; iter = iter + 1u32;
            } * (out.add((id as usize) as usize)) = k;
        }
    }
}
