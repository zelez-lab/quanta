...
#[doc = " corrections (1/12z and -1/360z³). Max relative error ~5e-4 at z=1"]
#[doc = " and falls off rapidly. Inputs z < 1 are clamped to 1 (the PTRD"]
#[doc = " acceptance test only queries z = k+1 with k ≥ 0)."]
#[allow(dead_code)] fn log_gamma_f32(z_in : f32) -> f32
{
    let z : f32 = if z_in < 1.0f32 { 1.0f32 } else { z_in }; let half_log_2pi
    : f32 = 0.918_938_5_f32; let log_z : f32 = z.ln(); let inv_z : f32 =
    1.0f32 / z; let inv_z3 : f32 = inv_z * inv_z * inv_z; (z - 0.5f32) * log_z
    - z + half_log_2pi + inv_z * (1.0f32 / 12.0f32) - inv_z3 *
    (1.0f32 / 360.0f32)
}
#[doc = " In-kernel Philox4×32-10, returning the first output word. Same"]
#[doc = " algorithm as `philox4x32::philox4x32_10_first_u32` in the host"]
#[doc = " API; transcribed here so the kernel macro can splice it into"]
#[doc = " the wasm shell."] #[doc = ""]
#[doc = " The round multiplies use the pure-32-bit mulhi form (see"]
#[doc = " `philox_mulhi32`) inlined into the loop body rather than a call:"]
#[doc = " this fn is imported cross-crate via `quanta::import_devices!`"]
#[doc = " and the splice is verbatim, so it must stay self-contained —"]
#[doc = " the same reason the round constants are local. No u64 appears"]
#[doc = " anywhere, so the kernel lowers on devices without `shaderInt64`."]
#[allow(dead_code)] fn
philox4x32_10_first_u32_kernel(c0 : u32, c1 : u32, c2 : u32, c3 : u32, k0 :
u32, k1 : u32) -> u32
{
    const M0_K : u32 = 0xD251_1F53; const M1_K : u32 = 0xCD9E_8D57; const W0_K
    : u32 = 0x9E37_79B9; const W1_K : u32 = 0xBB67_AE85; let mut x0 = c0; let
    mut x1 = c1; let mut x2 = c2; let mut x3 = c3; let mut key0 = k0; let mut
    key1 = k1; let mut i : u32 = 0; while i < 10u32
    {
        if i > 0
        { key0 = key0.wrapping_add(W0_K); key1 = key1.wrapping_add(W1_K); }
        let m0_lo : u32 = M0_K & 0xFFFFu32; let m0_hi : u32 = M0_K >> 16u32;
        let x0_lo : u32 = x0 & 0xFFFFu32; let x0_hi : u32 = x0 >> 16u32; let
        p0_lolo : u32 = m0_lo.wrapping_mul(x0_lo); let p0_lohi : u32 =
        m0_lo.wrapping_mul(x0_hi); let p0_hilo : u32 =
        m0_hi.wrapping_mul(x0_lo); let p0_hihi : u32 =
        m0_hi.wrapping_mul(x0_hi); let p0_cross : u32 =
        (p0_lolo >>
        16u32).wrapping_add(p0_lohi &
        0xFFFFu32).wrapping_add(p0_hilo & 0xFFFFu32); let hi0 : u32 =
        p0_hihi.wrapping_add(p0_lohi >>
        16u32).wrapping_add(p0_hilo >> 16u32).wrapping_add(p0_cross >> 16u32);
        let lo0 : u32 = M0_K.wrapping_mul(x0); let m1_lo : u32 = M1_K &
        0xFFFFu32; let m1_hi : u32 = M1_K >> 16u32; let x2_lo : u32 = x2 &
        0xFFFFu32; let x2_hi : u32 = x2 >> 16u32; let p1_lolo : u32 =
        m1_lo.wrapping_mul(x2_lo); let p1_lohi : u32 =
        m1_lo.wrapping_mul(x2_hi); let p1_hilo : u32 =
        m1_hi.wrapping_mul(x2_lo); let p1_hihi : u32 =
        m1_hi.wrapping_mul(x2_hi); let p1_cross : u32 =
        (p1_lolo >>
        16u32).wrapping_add(p1_lohi &
        0xFFFFu32).wrapping_add(p1_hilo & 0xFFFFu32); let hi1 : u32 =
        p1_hihi.wrapping_add(p1_lohi >>
        16u32).wrapping_add(p1_hilo >> 16u32).wrapping_add(p1_cross >> 16u32);
        let lo1 : u32 = M1_K.wrapping_mul(x2); let new_x0 = hi1 ^ x1 ^ key0;
        let new_x1 = lo1; let new_x2 = hi0 ^ x3 ^ key1; let new_x3 = lo0; x0 =
        new_x0; x1 = new_x1; x2 = new_x2; x3 = new_x3; i += 1;
    } x0
}

#[unsafe (no_mangle)] pub unsafe extern "C" fn
fill_poisson_u32_large(out : * mut u32, lambda : f32, seed_hi : u32, seed_lo :
u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let id = quark_id(); let lam : f32 = lambda; let smu : f32 =
            sqrt(lam); let b : f32 = 0.931f32 + 2.53f32 * smu; let a : f32 = -
            0.059f32 + 0.02483f32 * b; let inv_alpha : f32 = 1.1239f32 +
            1.1328f32 / (b - 3.4f32); let v_r : f32 = 0.9277f32 - 3.6224f32 /
            (b - 2.0f32); let log_lam : f32 = ln(lam); let log_inv_alpha : f32
            = ln(inv_alpha); let mut iter : u32 = 0u32; let mut result : u32 =
            0u32; let mut done : u32 = 0u32; while iter < 32u32 && done ==
            0u32
            {
                let r1 : u32 =
                philox4x32_10_first_u32_kernel(id, iter, 0u32, 0u32, seed_lo,
                seed_hi); let r2 : u32 =
                philox4x32_10_first_u32_kernel(id, iter, 1u32, 0u32, seed_lo,
                seed_hi); let u_bits : u32 = r1 >> 8u32; let v_bits : u32 = r2
                >> 8u32; let scale : f32 = 1.0f32 / 16_777_216.0f32; let u :
                f32 = (u_bits as f32) * scale - 0.5f32; let v : f32 =
                (v_bits as f32) * scale; let us : f32 = 0.5f32 - fabs(u); let
                k_f : f32 = floor((2.0f32 * a / us + b) * u + lam + 0.43f32);
                if k_f >= 0.0f32 && done == 0u32
                {
                    if us >= 0.07f32 && v <= v_r
                    { result = k_f as u32; done = 1u32; } else if !
                    (us < 0.013f32 && v > us)
                    {
                        let lhs : f32 = ln(v) + log_inv_alpha -
                        ln(a / (us * us) + b); let rhs : f32 = (0.0f32 - lam) +
                        (k_f * log_lam) - log_gamma_f32(k_f + 1.0f32); if lhs <= rhs
                        { result = k_f as u32; done = 1u32; }
                    }
                } iter = iter + 1u32;
            } * (out.add((id as usize) as usize)) = result;
        }
    }
}
