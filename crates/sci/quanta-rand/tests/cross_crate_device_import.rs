...
//
// Formerly the standalone `quanta-rand-import-test` crate; folded here as
// an integration test (which still compiles as its own crate and links
// `quanta_rand` externally, so the "downstream consumer" path is preserved
// — and `quanta_prims` remains a genuine external dev-dep). Two flavors,
// both end-to-end bit-exact:
//
// 1. **Explicit `import_devices!`**: user writes one line at file scope
//    listing the imports, then calls the device fn by bare name in the
//    kernel body.
// 2. **Auto-discovery**: user just writes `quanta_rand::foo(...)` inside
//    the kernel body. The kernel macro detects the qualified call,
//    rewrites it to bare-name, and emits the matching `_src!()` invocation
//    automatically.
