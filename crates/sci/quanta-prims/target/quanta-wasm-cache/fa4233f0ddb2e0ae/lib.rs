...
block_radix_sort_kv_u32_buffer(keys : * const u32, vals : * const u32,
keys_out : * mut u32, vals_out : * mut u32)
{
    ...
    {
        {
            ...
            (keys.add((i as usize) as usize)));
            shared_store_u32(1u32, (lane) as u32, *
            (vals.add((i as usize) as usize))); barrier(); let mut shift : u32
            = 0u32; while shift < 32u32
            {
                let my_key = shared_load_u32(0u32, (lane) as u32); let my_val
                = shared_load_u32(1u32, (lane) as u32); let digit =
                (my_key >> shift) & 3u32; let s_0 = if digit == 0u32 { 1u32 }
                else { 0u32 }; let s_1 = if digit == 1u32 { 1u32 } else
                { 0u32 }; let s_2 = if digit == 2u32 { 1u32 } else { 0u32 };
                ...
                shared_store_u32(1u32, (my_pos) as u32, my_val); barrier();
                shift = shift + 2u32;
            } * (keys_out.add((i as usize) as usize)) =
            shared_load_u32(0u32, (lane) as u32); *
            (vals_out.add((i as usize) as usize)) =
            shared_load_u32(1u32, (lane) as u32);
        }
    }
}
