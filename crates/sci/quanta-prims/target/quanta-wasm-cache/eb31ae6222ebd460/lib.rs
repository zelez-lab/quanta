...
block_sort_kv_u32_buffer(keys : * const u32, vals : * const u32, keys_out : *
mut u32, vals_out : * mut u32)
{
    ...
    {
        {
            ...
            (vals.add((i as usize) as usize))); barrier(); let mut k : u32 =
            2u32; while k <= 256u32
            {
                let mut j : u32 = k / 2u32; while j > 0u32
                {
                    let partner = lane ^ j; let my_key =
                    shared_load_u32(0u32, (lane) as u32); let partner_key =
                    shared_load_u32(0u32, (partner) as u32); let my_val =
                    shared_load_u32(1u32, (lane) as u32); let partner_val =
                    shared_load_u32(1u32, (partner) as u32); let bit_k = if
                    (lane & k) == 0u32 { 0u32 } else { 1u32 }; let bit_j = if
                    (lane & j) == 0u32 { 0u32 } else { 1u32 }; let take_partner
                    = if bit_k == bit_j { partner_key < my_key } else
                    { partner_key > my_key }; let new_key = if take_partner
                    { partner_key } else { my_key }; let new_val = if
                    take_partner { partner_val } else { my_val }; barrier();
                    shared_store_u32(0u32, (lane) as u32, new_key);
                    shared_store_u32(1u32, (lane) as u32, new_val); barrier(); j
                    = j / 2u32;
                } k = k * 2u32;
