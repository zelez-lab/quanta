...
global_bitonic_pass_u32(data : * mut u32, k : u32, j : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let partner = i ^ j; if partner > i
            {
                let a = * (data.add((i as usize) as usize)); let b = *
                (data.add((partner as usize) as usize)); let bit_k = if
                (i & k) == 0u32 { 0u32 } else { 1u32 }; let out_of_order = if
                bit_k == 0u32 { b < a } else { b > a }; if out_of_order
                {
                    * (data.add((i as usize) as usize)) = b; *
                    (data.add((partner as usize) as usize)) = a;
                }
            }
        }
    }
