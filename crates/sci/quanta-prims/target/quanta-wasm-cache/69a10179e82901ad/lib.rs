...
block_compact_u32_buffer(predicates : * const u32, data : * const u32, out : *
mut u32, counts : * mut u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let lane = proton_id(); let block =
            nucleus_id(); if lane < 32u32
            { shared_store_u32(0u32, (lane) as u32, 0u32); } barrier(); let
            pred = * (predicates.add((i as usize) as usize)); let inclusive =
            block_scan_add_u32_kernel(pred); let exclusive = inclusive - pred;
            let block_start = block * 256u32; if pred != 0u32
            {
                * (out.add(((block_start + exclusive) as usize) as usize)) = *
                (data.add((i as usize) as usize));
            } if lane == 255u32
            { * (counts.add((block as usize) as usize)) = inclusive; }
        }
    }
}
