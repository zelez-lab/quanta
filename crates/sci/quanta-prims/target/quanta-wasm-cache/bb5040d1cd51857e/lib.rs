...
block_histogram_u32_buffer(buckets_in : * const u32, counts_out : * mut u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let lane = proton_id(); let block =
            nucleus_id(); shared_store_u32(0u32, (lane) as u32, 0u32);
            barrier(); let bucket = * (buckets_in.add((i as usize) as usize));
            unsafe { atomic_add_shared_u32(0u32, bucket, 1u32, 0u32); }
            barrier(); *
            (counts_out.add(((block * 256u32 + lane) as usize) as usize)) =
