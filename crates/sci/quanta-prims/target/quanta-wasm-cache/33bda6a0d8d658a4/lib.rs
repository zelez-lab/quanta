...
block_reduce_max_u32_tree_buffer(data : * const u32, out : * mut u32)
{
    ...
    {
        {
            ...
            128u32; while s > 0u32
            {
                let v : u32 = shared_load_u32(0u32, (lane) as u32); let p = if
                lane < s { lane + s } else { lane }; let o : u32 =
                shared_load_u32(0u32, (p) as u32); let nv = if lane < s
                { if o > v { o } else { v } } else { v }; barrier();
                shared_store_u32(0u32, (lane) as u32, nv); barrier(); s = s /
                2u32;
            } if lane == 0u32
            {
                * (out.add((block as usize) as usize)) =
                shared_load_u32(0u32, (0u32) as u32);
            }
        }
    }
}
