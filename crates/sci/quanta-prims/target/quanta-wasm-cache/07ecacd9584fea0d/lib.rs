...
block_top_k_u32_buffer(data : * const u32, top_k_out : * mut u32, k : u32)
{
    ...
    {
        {
            ...
            (data.add((i as usize) as usize))); barrier(); let mut outer : u32
            = 2u32; while outer <= 256u32
            {
                let mut inner : u32 = outer / 2u32; while inner > 0u32
                {
                    let partner = lane ^ inner; let my_key =
                    shared_load_u32(0u32, (lane) as u32); let partner_key =
                    shared_load_u32(0u32, (partner) as u32); let bit_k = if
                    (lane & outer) == 0u32 { 0u32 } else { 1u32 }; let bit_j =
                    if (lane & inner) == 0u32 { 0u32 } else { 1u32 }; let
                    take_partner = if bit_k == bit_j { partner_key < my_key }
                    else { partner_key > my_key }; let new_key = if take_partner
                    { partner_key } else { my_key }; barrier();
                    shared_store_u32(0u32, (lane) as u32, new_key); barrier();
                    inner = inner / 2u32;
                } outer = outer * 2u32;
            } if lane < k
            {
                * (top_k_out.add(((block * k + lane) as usize) as usize)) =
                shared_load_u32(0u32, ((255u32 - lane) as usize) as u32);
            }
        }
    }
}
