...
#[doc = " Block-wide f32 max reduction. Result in lane 0 of the"]
#[doc = " workgroup. Identity element: f32::NEG_INFINITY. See"]
#[doc = " module-level docs for caller contract."]
#[allow(dead_code, unused_unsafe)] pub fn
block_reduce_max_f32_kernel(value : f32) -> f32
{
    let warp_max = unsafe { reduce_max_f32(value) }; let sub_size = unsafe
    { subgroup_size() }; let lane_in_block = unsafe { proton_id() }; let
    lane_in_warp = lane_in_block % sub_size; let warp_id = lane_in_block /
    sub_size; if lane_in_warp == 0
    { unsafe { shared_store_f32(0, warp_id, warp_max) }; } unsafe
    { barrier() }; let partial = unsafe { shared_load_f32(0, lane_in_warp) };
    unsafe { reduce_max_f32(partial) }
}

#[unsafe (no_mangle)] pub unsafe extern "C" fn
block_reduce_max_f32_buffer(data : * const f32, out : * mut f32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let block = nucleus_id(); let lane =
            proton_id(); if lane < 32u32
            { shared_store_f32(0u32, (lane) as u32, - 1.0e38f32); } barrier();
            let value = * (data.add((i as usize) as usize)); let block_max =
            block_reduce_max_f32_kernel(value); if lane == 0u32
            { * (out.add((block as usize) as usize)) = block_max; }
        }
    }
}
