...
block_segmented_sort_u32_buffer(data : * const u32, flags : * const u32, out :
* mut u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let lane = proton_id();
            shared_store_u32(0u32, (lane) as u32, *
            (data.add((i as usize) as usize))); let mut f0 : u32 = *
            (flags.add((i as usize) as usize)); if lane == 0u32 { f0 = 1u32; }
            shared_store_u32(2u32, (lane) as u32, f0); barrier(); let mut ds :
            u32 = 1u32; while ds < 256u32
            {
                let mut c_new : u32 = shared_load_u32(2u32, (lane) as u32); if
                lane >= ds
                { c_new = c_new + shared_load_u32(2u32, (lane - ds) as u32); }
                barrier(); shared_store_u32(2u32, (lane) as u32, c_new);
                barrier(); ds = ds * 2u32;
            }
            shared_store_u32(1u32, (lane) as u32,
            shared_load_u32(2u32, (lane) as u32) - 1u32); barrier(); let mut
            pass : u32 = 0u32; while pass < 20u32
            {
                let my_key = shared_load_u32(0u32, (lane) as u32); let my_seg
                = shared_load_u32(1u32, (lane) as u32); let digit = if pass <
                16u32 { (my_key >> (pass * 2u32)) & 3u32 } else
                { (my_seg >> ((pass - 16u32) * 2u32)) & 3u32 }; let s_0 = if
                digit == 0u32 { 1u32 } else { 0u32 }; let s_1 = if digit ==
                1u32 { 1u32 } else { 0u32 }; let s_2 = if digit == 2u32
                { 1u32 } else { 0u32 }; let s_3 = if digit == 3u32 { 1u32 }
                else { 0u32 };
                shared_store_u32(2u32, (lane) as u32, s_0 + s_1 * 65536u32);
                shared_store_u32(3u32, (lane) as u32, s_2 + s_3 * 65536u32);
                barrier(); let mut d : u32 = 1u32; while d < 256u32
                {
                    let mut v01 : u32 = shared_load_u32(2u32, (lane) as u32);
                    let mut v23 : u32 = shared_load_u32(3u32, (lane) as u32); if
                    lane >= d
                    {
                        v01 = v01 + shared_load_u32(2u32, (lane - d) as u32); v23 =
                        v23 + shared_load_u32(3u32, (lane - d) as u32);
                    } barrier(); shared_store_u32(2u32, (lane) as u32, v01);
                    shared_store_u32(3u32, (lane) as u32, v23); barrier(); d = d
                    * 2u32;
                } let t01 = shared_load_u32(2u32, (255u32) as u32); let t23 =
                shared_load_u32(3u32, (255u32) as u32); let total0 = t01 &
                65535u32; let total1 = t01 >> 16u32; let total2 = t23 &
                65535u32; let base1 = total0; let base2 = base1 + total1; let
                base3 = base2 + total2; let inc01 =
                shared_load_u32(2u32, (lane) as u32); let inc23 =
                shared_load_u32(3u32, (lane) as u32); let rank0 =
                (inc01 & 65535u32).wrapping_sub(1u32); let rank1 = base1 +
                (inc01 >> 16u32).wrapping_sub(1u32); let rank2 = base2 +
                (inc23 & 65535u32).wrapping_sub(1u32); let rank3 = base3 +
                (inc23 >> 16u32).wrapping_sub(1u32); let my_pos : u32 = s_0 *
                rank0 + s_1 * rank1 + s_2 * rank2 + s_3 * rank3; barrier();
                shared_store_u32(0u32, (my_pos) as u32, my_key);
                shared_store_u32(1u32, (my_pos) as u32, my_seg); barrier();
                pass = pass + 1u32;
            } * (out.add((i as usize) as usize)) =
            shared_load_u32(0u32, (lane) as u32);
        }
    }
}
