...
block_reduce_min_f32_tree_buffer(data : * const f32, out : * mut f32)
{
    ...
    {
        {
            ...
            nucleus_id();
            shared_store_f32(0u32, (lane) as u32, *
            (data.add((i as usize) as usize))); barrier(); let mut s : u32 =
            128u32; while s > 0u32
            {
                let v : f32 = shared_load_f32(0u32, (lane) as u32); let p = if
                lane < s { lane + s } else { lane }; let o : f32 =
                shared_load_f32(0u32, (p) as u32); let nv = if lane < s
                { if o < v { o } else { v } } else { v }; barrier();
                shared_store_f32(0u32, (lane) as u32, nv); barrier(); s = s /
                2u32;
            } if lane == 0u32
            {
                * (out.add((block as usize) as usize)) =
                shared_load_f32(0u32, (0u32) as u32);
            }
        }
    }
}
