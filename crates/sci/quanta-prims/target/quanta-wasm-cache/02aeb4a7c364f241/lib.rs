...
block_segmented_reduce_add_u32_buffer(data : * const u32, flags : * const u32,
totals_out : * mut u32, counts_out : * mut u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let lane = proton_id(); let block =
            nucleus_id();
            ...
            shared_store_u32(1u32, (lane) as u32, f0);
            shared_store_u32(2u32, (lane) as u32, f0); barrier(); let mut d :
            u32 = 1u32; while d < 256u32
            {
                let mut v_new : u32 = shared_load_u32(0u32, (lane) as u32);
                let mut f_new : u32 = shared_load_u32(1u32, (lane) as u32);
                let mut c_new : u32 = shared_load_u32(2u32, (lane) as u32); if
                lane >= d
                {
                    let v_prev = shared_load_u32(0u32, (lane - d) as u32); let
                    f_prev = shared_load_u32(1u32, (lane - d) as u32); if f_new
                    == 0u32 { v_new = v_new + v_prev; } f_new = f_new | f_prev;
                    c_new = c_new + shared_load_u32(2u32, (lane - d) as u32);
                } barrier(); shared_store_u32(0u32, (lane) as u32, v_new);
                shared_store_u32(1u32, (lane) as u32, f_new);
                shared_store_u32(2u32, (lane) as u32, c_new); barrier(); d = d
                * 2u32;
            } let my_count = shared_load_u32(2u32, (lane) as u32); let is_last
            : u32 = if lane == 255u32 { 1u32 } else if
            shared_load_u32(2u32, (lane + 1u32) as u32) == my_count + 1u32
            { 1u32 } else { 0u32 }; if is_last == 1u32
            {
                *
                (totals_out.add(((block * 256u32 + (my_count - 1u32)) as
                usize) as usize)) = shared_load_u32(0u32, (lane) as u32);
            } if lane == 255u32
            { * (counts_out.add((block as usize) as usize)) = my_count; }
        }
    }
}
