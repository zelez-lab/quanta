...
#[doc = " Block-wide f32 inclusive prefix-sum scan."]
#[allow(dead_code, unused_unsafe)] pub fn
block_scan_add_f32_kernel(value : f32) -> f32
{
    let warp_inc = unsafe { scan_add_f32(value) }; let sub_size = unsafe
    ...
    { unsafe { shared_store_f32(0, warp_id, warp_inc) }; } unsafe
    { barrier() }; if warp_id == 0u32
    {
        let total = unsafe { shared_load_f32(0, lane_in_warp) }; let prefix =
        unsafe { scan_add_exclusive_f32(total) }; unsafe
        { shared_store_f32(0, lane_in_warp, prefix) };
    } unsafe { barrier() }; let warp_offset = unsafe
    { shared_load_f32(0, warp_id) }; warp_inc + warp_offset
}

#[unsafe (no_mangle)] pub unsafe extern "C" fn
block_scan_add_f32_buffer(data : * const f32, out : * mut f32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let lane = proton_id(); if lane < 32u32
            { shared_store_f32(0u32, (lane) as u32, 0.0f32); } barrier(); let
            value = * (data.add((i as usize) as usize)); let scan_result =
            block_scan_add_f32_kernel(value); *
