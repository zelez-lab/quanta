//! Narrow-int (u8 / i8 / u16 / i16) storage contract on the AOT SPIR-V
//! emitter.
//!
//! Mirrors `quanta-ir/tests/emit_spirv_narrow_int_storage.rs` for the
//! JIT emitter — the twins share one storage contract. Narrow-int
//! buffers store native-stride 8-/16-bit elements (ArrayStride 1/2)
//! through the same StorageBuffer8/16BitAccess seam as fp8/bf16, while
//! registers stay the canonical unified u32. The module must declare
//! the matching storage-access capability (and `SPV_KHR_8bit_storage`
//! for the 8-bit types), widen every load into the u32 register
//! (OpUConvert; signed types then sign-extend via shl + arithmetic-shr
//! — the CPU interpreter's widen-on-load semantics), and truncate every
//! store back to the storage element (OpUConvert, ≡ mod 2^w — the
//! wrapping contract). These tests pin the instruction sequences by
//! decoding the word stream, and run the reference validator armed with
//! `--target-env` when `spirv-val` is on PATH.
...
fn has_capability(insts: &[(u16, Vec<u32>)], cap: u32) -> bool {
    insts
        .iter()
...
fn has_extension(insts: &[(u16, Vec<u32>)], name: &str) -> bool {
    insts
        .iter()
        .any(|(op, args)| *op == OP_EXTENSION && decode_string(args) == name)
}

fn array_strides(insts: &[(u16, Vec<u32>)]) -> Vec<u32> {
    insts
        .iter()
...
fn type_int(insts: &[(u16, Vec<u32>)], width: u32) -> Option<u32> {
...
fn constant_u32(insts: &[(u16, Vec<u32>)], val: u32) -> Option<u32> {
...
fn result_of(insts: &[(u16, Vec<u32>)], opcode: u16, pred: impl Fn(&[u32]) -> bool) -> Option<u32> {
    insts.iter().find_map(|(op, a)| {
...
fn assert_widen_chain(insts: &[(u16, Vec<u32>)], name: &str, bits: u32, signed: bool) -> u32 {
...
fn assert_truncate_store(insts: &[(u16, Vec<u32>)], name: &str, bits: u32) -> u32 {
    let narrow_ty = type_int(insts, bits)
        .unwrap_or_else(|| panic!("{name}: module must declare OpTypeInt {bits} 0"));
...
fn narrow_int_buffers_use_native_stride_and_declare_capabilities() {
    for (ty, bits, signed, stride, tag) in NARROW_INTS {
        ...
        let spirv = crate::emit_spirv::emit(&roundtrip_kernel(ty, &name)).unwrap();
        let w = words(&spirv);
...
fn signed_compare_orders_on_the_sign_extended_register() {
    ...
    for (ty, bits, signed, _, tag) in NARROW_INTS {
        ...
        let spirv = crate::emit_spirv::emit(&cmp_kernel(ty, &name)).unwrap();
        let w = words(&spirv);
        let insts = instructions(&w);
...
fn cast_of_negative_one_reduces_then_stores_the_narrow_pattern() {
    ...
    for (to, bits, _, _, tag) in [
        ...
        let spirv = crate::emit_spirv::emit(&cast_neg1_kernel(to, &name)).unwrap();
        let w = words(&spirv);
        let insts = instructions(&w);
...
fn mixed_narrow_and_wide_kernel_carries_all_strides() {
    let spirv = crate::emit_spirv::emit(&mixed_kernel("mixed_widths")).unwrap();
    let w = words(&spirv);
    let insts = instructions(&w);
...
fn accumulator_loop_over_narrow_loads_validates() {
    ...
    for (ty, bits, signed, _, tag) in NARROW_INTS {
        ...
        let spirv = crate::emit_spirv::emit(&accum_kernel(ty, &name)).unwrap();
        let w = words(&spirv);
        let insts = instructions(&w);
        assert_widen_chain(&insts, &name, bits, signed);
        spirv_val(&name, &spirv);
...
fn narrow_satadd_clamps_at_the_type_bound() {
    ...
    // not the wide wrap test `sum < a`. Mirrors the JIT decode test.
    for (ty, max, tag) in [
        (ScalarType::U8, 0xFFu32, "u8"),
        (ScalarType::U16, 0xFFFF, "u16"),
    ] {
        let name = format!("{tag}_satadd");
        let spirv = crate::emit_spirv::emit(&binop_kernel(BinOp::SatAdd, ty, &name)).unwrap();
        let w = words(&spirv);
        let insts = instructions(&w);
    ...
    // the 32-bit max.
    let spirv =
        crate::emit_spirv::emit(&binop_kernel(BinOp::SatAdd, ScalarType::U32, "u32_satadd"))
            .unwrap();
    let w = words(&spirv);
    let insts = instructions(&w);
    assert!(
...
fn narrow_rotate_masks_and_reextends_signed_carriers() {
    ...
    // zero-extended: no width mask, no arithmetic shifts. Mirrors
    // the JIT decode test.
    for (sty, uty, mask, dist, tag) in [
        ...
        for op in [BinOp::Rotl, BinOp::Rotr] {
            ...
            let spirv = crate::emit_spirv::emit(&binop_kernel(op, sty, &name)).unwrap();
            let w = words(&spirv);
            let insts = instructions(&w);
            ...
            let spirv = crate::emit_spirv::emit(&binop_kernel(op, uty, &uname)).unwrap();
            let w = words(&spirv);
            let insts = instructions(&w);
            assert!(
                !insts.iter().any(|(op, _)| *op == OP_SHIFT_RIGHT_ARITHMETIC),
