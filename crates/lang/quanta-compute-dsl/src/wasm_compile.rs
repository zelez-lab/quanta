...
///
/// `helper_sources` is a list of `fn` items (as source strings) that
/// the kernel calls — each helper is spliced into the wrapper crate
/// alongside the kernel decl, so `call $helper_name` resolves at
/// rustc-compile time. Helpers may transitively call each other;
/// they're emitted in a flat module scope so forward-refs work.
pub(crate) fn compile_kernel_to_wasm(
    kernel_decl: &str,
    helper_sources: &[String],
) -> Result<Vec<u8>, String> {
    let wrapped = wrap_kernel_source(kernel_decl, helper_sources);
...
///
/// `helper_sources` are `fn` items emitted at the crate root with
/// `use crate::quanta::intrinsics::*` already in scope, so they can
/// call any kernel-side intrinsic just like the kernel body itself.
fn wrap_kernel_source(kernel_decl: &str, helper_sources: &[String]) -> String {
    let intrinsics = strip_intrinsics_top_attrs(INTRINSICS_SRC);
    let helpers_joined = helper_sources.join("\n");
    format!(
