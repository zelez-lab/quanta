...
# [unsafe (no_mangle)] pub unsafe extern "C" fn mandelbrot (output : * mut u32 , height : u32 , max_iter : u32 , width : u32) { # [allow (unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe { { let idx = quark_id () ; let px = idx % width ; let py = idx / width ; let x0 = (px as f32 / width as f32) * 3.5f32 - 2.5f32 ; let y0 = (py as f32 / height as f32) * 2.0f32 - 1.0f32 ; let mut x = 0.0f32 ; let mut y = 0.0f32 ; let mut iter = 0u32 ; while x * x + y * y <= 4.0f32 && iter < max_iter { let tmp = x * x - y * y + x0 ; y = 2.0f32 * x * y + y0 ; x = tmp ; iter += 1u32 ; } * (output . add ((idx) as usize)) = iter ; } } }
