//! Integer division/remainder-by-zero guard on the MSL emitter.
//!
//! The CPU reference defines x/0 = 0 and x%0 = 0 for every int width;
//! Metal's hardware returns ~0 for u32 x/0 (measured on M-series), so
//! the emitter must guard. A plain result-side ternary is NOT enough —
//! SIMD may evaluate both sides — so the DIVISOR is substituted before
//! the divide, then the result selected:
//!
...
use quanta_ir::{BinOp, KernelDef, KernelOp, KernelParam, Reg, ScalarType, emit_msl};
...
fn int_div_and_rem_emit_the_divisor_substituting_ternary() {
    for (ty, op, o, t, tag) in [
        ...
        let msl = emit_msl::emit(&binop_kernel(ty, op, tag)).unwrap();
        let want = format!(
            "{t} r3 = (({t})r2 == ({t})0) ? ({t})0 : \
...
fn float_div_stays_bare_ieee() {
    ...
    let msl = emit_msl::emit(&binop_kernel(ScalarType::F32, BinOp::Div, "f32_div")).unwrap();
    assert!(
