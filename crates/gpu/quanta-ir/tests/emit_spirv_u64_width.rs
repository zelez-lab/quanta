...
//!
//! `fill_uniform_f64` packs two Philox u32 draws into a u64 —
//! `((hi as u64) << 32) | lo`, then `>> 11` and a u64→f64 convert. The SPIR-V
//! emitters used to lower this at 32-bit width (the AOT emitter collapsed
//! `ScalarType::U64` to `%uint`; the JIT emitter spelled the width-changing
//! u32→u64 cast as an `OpBitcast`, which cannot change total bit width). On
//! the AOT path the high word shifted out to zero, so every f64 draw was ~0 —
//! a silent miscompile on Vulkan (lavapipe: mean=0.0, var=0.0). These tests
//! pin the emitted module shape: `spirv-val` accepts it AND the pack/shift
//! chain is typed `OpTypeInt 64 0`.
...
/// Assert the pack/shift chain of `spirv` runs at 64-bit width: the Int64
/// capability is declared, an unsigned 64-bit int type exists, the shifts
/// and the OR produce that type, and the u64→f64 convert consumes a
/// 64-bit-typed value.
fn assert_u64_chain_is_64_bit(spirv: &[u8]) {
    ...
    // The u64→f64 convert must consume a value defined at 64-bit width.
    let defs: std::collections::HashMap<u32, u32> = instrs
        .iter()
        .filter(|(_, args)| args.len() >= 2)
        .map(|(_, args)| (args[1], args[0]))
        .collect();
    let converts: Vec<&Vec<u32>> = instrs
        .iter()
        .filter(|(op, _)| *op == OP_CONVERT_U_TO_F)
        .map(|(_, args)| args)
        .collect();
    assert!(!converts.is_empty(), "expected an OpConvertUToF");
    for args in converts {
        let src_ty = defs.get(&args[2]).copied();
        assert_eq!(
            src_ty.map(|t| ulong.contains(&t)),
            Some(true),
            "OpConvertUToF must consume a 64-bit value (src type {src_ty:?})"
        );
    }
}
...
fn u64_pack_shape_is_valid_and_64_bit_wide() {
    let spirv = emit_spirv::emit(&u64_pack_kernel()).expect("emit");
    assert_spirv_val(&spirv);
    assert_u64_chain_is_64_bit(&spirv);
}

/// Sign-extension sibling: `(i32 as i64)` must be a real OpSConvert chain,
/// and truncation `(u64 as u32)` a real OpUConvert — never OpBitcast.
fn i64_extend_truncate_kernel() -> KernelDef {
    let body = vec![
        KernelOp::QuarkId { dst: Reg(0) },
        KernelOp::Const {
            dst: Reg(1),
...
#[test]
fn i64_sign_extend_and_truncate_are_valid_spirv() {
    let spirv = emit_spirv::emit(&i64_extend_truncate_kernel()).expect("emit");
    assert_spirv_val(&spirv);
}
