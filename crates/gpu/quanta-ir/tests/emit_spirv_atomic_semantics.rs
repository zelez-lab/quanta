//! Memory-semantics words across the SPIR-V emitter's atomic/fence sites.
//!
//! Two invariants, both Vulkan environment rules the emitter cannot show
//! locally:
//!
//! 1. No emitted semantics word carries SequentiallyConsistent (0x10) —
//!    Vulkan forbids it outright (VUID-StandaloneSpirv-None-04732), so
//!    `SeqCst` clamps to AcquireRelease.
...
//!    Release → None and AcqRel/SeqCst → Acquire — the same clamp the LLVM
//!    emitter applies to `cmpxchg`'s failure ordering.
//!
//! The decoded operand pairs are asserted per `MemoryOrder`, and every
//! module is pushed through `spirv-val --target-env vulkan1.3` where the
//! validator is installed (plain `spirv-val` does NOT check the Vulkan
//! rules — the target env is what arms VUID-04732).
...
fn const_values(instrs: &[(u16, Vec<u32>)]) -> HashMap<u32, u32> {
    instrs
        .iter()
        .filter(|(op, operands)| *op == OP_CONSTANT && operands.len() == 3)
...
fn cas_semantics_pair(spv: &[u8]) -> (u32, u32) {
    ...
    let cas: Vec<_> = instrs
        .iter()
        .filter(|(op, _)| *op == OP_ATOMIC_COMPARE_EXCHANGE)
        .collect();
...
/// Run `spirv-val --target-env vulkan1.3` if it is on PATH; `None` = tool
/// not installed (decoded-operand asserts carry the test).
fn validate_vulkan(name: &str, spv: &[u8]) {
    let dir = std::env::temp_dir().join("quanta_atomic_semantics_test");
...
fn cas_semantics_split_and_clamped() {
    ...
    for (success, failure, want_eq, want_uneq) in cases {
        let spv = quanta_ir::emit_spirv::emit(&cas_kernel(success, failure)).expect("emit");
...
fn rmw_semantics_never_seq_cst() {
    ...
    for (order, want) in cases {
        let spv = quanta_ir::emit_spirv::emit(&rmw_kernel(order)).expect("emit");
        let words = to_words(&spv);
        let instrs = instructions(&words);
        let consts = const_values(&instrs);
        let adds: Vec<_> = instrs
            .iter()
            .filter(|(op, _)| *op == OP_ATOMIC_IADD)
            .collect();
...
fn fence_semantics_never_seq_cst() {
    ...
    for (order, want_bits) in cases {
        let spv = quanta_ir::emit_spirv::emit(&fence_kernel(order)).expect("emit");
        let words = to_words(&spv);
        let instrs = instructions(&words);
        let consts = const_values(&instrs);
        let fences: Vec<_> = instrs
            .iter()
            .filter(|(op, _)| *op == OP_MEMORY_BARRIER)
            .collect();
    ...
    let spv = quanta_ir::emit_spirv::emit(&fence_kernel(MemoryOrder::Relaxed)).expect("emit");
    let instrs = instructions(&to_words(&spv));
    assert!(
        instrs.iter().all(|(op, _)| *op != OP_MEMORY_BARRIER),
...
fn shared_cas_failure_clamped() {
    let spv = quanta_ir::emit_spirv::emit(&shared_cas_kernel(MemoryOrder::SeqCst)).expect("emit");
    let (got_eq, got_uneq) = cas_semantics_pair(&spv);
    assert_eq!(
        (got_eq, got_uneq),
