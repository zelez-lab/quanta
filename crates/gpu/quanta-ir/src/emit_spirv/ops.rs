...
impl SpvEmitter {
        ...
        match op {
            ...
            KernelOp::Load {
                ...
                if index.0 == u32::MAX {
                    ...
                    // alignment (2 for bf16/u16/i16, 1 for fp8/u8/i8), then
                    // widen to the body type (u16/u8 → f32 for the narrow
                    // floats, → canonical u32 for the narrow ints).
            ...
            KernelOp::Store {
                ...
                // element type: OpSelect over an 8-/16-bit element would
                ...
                if src_ty == bool_ty && elem_ty != bool_ty {
                    ...
                    val = self.coerce_to(val, bool_ty, body_ty);
                }
                ...
                // bf16/fp8: pack the f32 body value into its storage bits.
            ...
            KernelOp::BinOp { dst, a, b, op, ty } => {
                ...
                if matches!(op, BinOp::Rotl | BinOp::Rotr) {
                    ...
                    // Signed narrow values ride the carrier
                    // SIGN-extended; the decomposition's logical right
                    // shift would drag the extension bits into the low
                    // w bits (i8 rotl(0xAA, 2) would come out 0xFE
                    // instead of 0xAA). Mask the value to its width
                    // first, and re-extend the result below so the
                    // register stays canonical. Unsigned narrow
                    // carriers are already zero-extended — no mask
                    // needed.
                    ...
                    // Re-canonicalize signed narrow results: mask to
                    // the width and sign-extend (the widen helper's
                    // shl + arithmetic-shr does both).
                ...
                // Saturation clamps at the TYPE's bounds:
                //   satadd(a,b) = let s = a+b in
                //     wide:   (s < a) ? MAX : s          (32-/64-bit wrap)
                //     narrow: (s > MAX_w) ? MAX_w : s    (carrier can't wrap)
                //   satsub(a,b) = (a < b) ? 0 : a-b      (underflow — the
                //     0 clamp is already the narrow bound)
                // Narrow values ride the canonical u32 carrier, so their
                // 32-bit sum never wraps; the clamp compares against the
                // narrow max instead, and the store's truncation is a
                // no-op on the clamped value.
                if matches!(op, BinOp::SatAdd | BinOp::SatSub) && !is_float && !is_signed {
                    let result = if matches!(op, BinOp::SatAdd) {
                        ...
                        if matches!(ty, ScalarType::U8 | ScalarType::U16) {
                            // overflow ⇔ sum > MAX_w (unsigned)
                            Self::emit_op(
                                &mut self.sec_function,
                                OP_UGREATER_THAN,
                                &[bool_ty, overflowed, sum, max_val],
                            );
                        } else {
                ...
                // stays as-is (excluded in the op-matrix).
                if !is_float && matches!(op, BinOp::Div | BinOp::Rem) {
                    let bool_ty = self.ensure_type_bool();
                    let zero = self.emit_constant_typed_zero(*ty);
            ...
            KernelOp::Cast { dst, src, from, to } => {
                ...
                // the documented non-portable case).
                let src_ty = self.reg_type_id(*src)?;
                if let (false, false, Some((bits, signed))) = (
                    from_float,
                    matches!(from, ScalarType::Bool),
                    narrow_int_info(to),
                ) {
                    let u32_ty = self.ensure_type_u32();
    ...
    /// pinned to 0.0. The device's compute sampler (NEAREST, CLAMP_TO_EDGE,
    /// unnormalized) makes the fetched texel match the CPU executor exactly.
    pub(crate) fn emit_op_texture_sample_2d(
        ...
        if let Some(&(var_id, type_id)) = self.texture_samplers.get(&texture) {
            ...
            // texel indices (`quark_id()`-derived), so convert each to f32 —
            // OpImageSample* takes a float coordinate vector, and a raw
            // int-vector constituent would fail spirv-val. With the unnormalized
            // compute sampler these floats are texel coordinates, not [0,1] UVs.
