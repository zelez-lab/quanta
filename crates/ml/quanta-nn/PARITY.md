...
| Item | Status | Notes |
|---|---|---|
| `Linear` | **shipped (Layer)** | `layer::Linear` — `y = x @ w (+ b)`, `w: [in, out]`; gemm-backed through `Var::matmul` in BOTH directions (the VJP's two products are gemms); optional bias; kaiming-uniform default delegating to `init::Init::KaimingUniform`. Bias fusion MEASURED AND DECLINED (2026-07): the epilogue would save ~1 broadcast-add encode per direction per Linear; under the hazard-run lane an encode is ~5-15 µs with no extra flush, so a full transformer layer saves ~0.1 ms/step — noise against the ~46 s suite. Revisit only if a profile ever shows the adds mattering. |
...
| `TransformerEncoderLayer` | **shipped (Layer)** | `transformer::TransformerEncoderLayer` — pre-LN as THE form (no `norm_first` flag): `x + Dropout(MHA(LN₁x))` then `+ Dropout(W₂·SwiGLU(W₁·LN₂h))`; every piece a shipped proven citizen. Full `Layer`: tuple-stackable, `apply_train` threads one key through both dropouts, derived five-subtree params checkpoint by name. `examples/cookbook_transformer.rs` = SUMMIT A (Embedding → causal+rotary block → CE → Adam → named checkpoint → greedy generation). Deliberately small dims in tests/example. The per-op host-sync cost is CLOSED: every array/nn dispatch now encodes into the device's deferred lane (deferral is the default dispatch model) and submits in batches at host boundaries — the transformer test file went ~9.5 min → ~5 s on M1 Pro (deferred lane + the f32 zero-copy fused-op path: inputs bind their own buffers, outputs are adopted, backward closures capture device fields). Dependency-aware batch encoding landed (hazard-run grouping over Wave write-masks; Metal concurrent encoder, Vulkan run-boundary barriers) and sdpa is device-resident under it — full nn suite ≈ 46 s. Remaining recorded increments: per-op wave JIT (~10 µs), output-field pooling. |
...
| safetensors interop | **shipped** — `safetensors::{save, load, save_named, load_named}`: the HF format, dependency-free (hand-rolled header JSON per the no-wrapper-crates policy, byte-exact against a hand-built spec reference). Saves `F32`; loads `F32` exact + `F16`/`BF16` upconverted (incl. subnormals) — other dtypes are loud errors naming the tensor. Tree forms ride `named_flatten`: NAME-keyed load with the `load_state` missing/extra/shape contract; `__metadata__` round-trips. |
| npy / npz interop | **shipped** in `quanta-array` (`npy`/`npz` modules, `NPY_INTEROP.md`) — plain-array IO landed beside the arrays, as ruled |
...
- Data loading: step 084.8 (quanta-data lane). Tokenizers: **shipped** —
  the standalone `quanta-tokenizers` crate (`TOKENIZER_CONTRACT.md` at
  its root is the declared surface); the bridge is
  `Array::from_slice(&gpu, encoding.ids(), &[n])` into `Embedding`.
