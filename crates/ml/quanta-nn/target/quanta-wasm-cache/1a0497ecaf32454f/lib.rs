    ...
    {
        {
            ...
            q_base = qi * d; let mut dot0 : f32 = 0.0f32; let mut p0 : u32 =
            0u32; while p0 < d
            {
                dot0 = dot0 + * (q.add(((q_base + p0) as usize) as usize)) * *
                (k.add((p0 as usize) as usize)); p0 = p0 + 1u32;
            } let s0 = scale * dot0; let mut m : f32 = s0; let mut l : f32 =
            1.0f32; let mut acc : f32 = * (v.add((vd as usize) as usize)); let
            mut kj : u32 = 1u32; while kj < seq_k
            {
                ...
                } let raw = scale * dot; let causal_ok = if causal == 0u32
                { 1.0f32 } else if kj <= qi { 1.0f32 } else { 0.0f32 }; let
                pad_ok = if kj < kv_len { 1.0f32 } else { 0.0f32 }; let valid
                = causal_ok * pad_ok; let sk = raw + (1.0f32 - valid) *
                (0.0f32 - 1000000000.0f32); let m_new = fmax(m, sk); let a =
                exp(m - m_new); let b = exp(sk - m_new); l = l * a + b; acc =
                acc * a + b * * (v.add(((kj * dv + vd) as usize) as usize)); m
                = m_new; kj = kj + 1u32;
