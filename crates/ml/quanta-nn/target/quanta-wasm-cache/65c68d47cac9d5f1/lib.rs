...
sdpa_forward(q : * const f32, k : * const f32, v : * const f32, out : * mut
f32, stats : * mut f32, seq_q : u32, seq_k : u32, d : u32, dv : u32, scale :
f32, causal : u32, kv_len : u32)
{
    #[allow(unused_imports)] use crate :: quanta :: intrinsics :: * ; unsafe
    {
        {
            let i = quark_id(); let total = seq_q * dv; let idx = if i < total
            { i } else { 0u32 }; let qi = idx / dv; let vd = idx % dv; let
            q_base = qi * d; let mut m : f32 = 0.0f32; let mut l : f32 =
            0.0f32; let mut acc : f32 = 0.0f32; let mut kj : u32 = 0u32; while
            kj < seq_k
            {
                let k_base = kj * d; let mut dot : f32 = 0.0f32; let mut p :
                u32 = 0u32; while p < d
                {
                    dot = dot + * (q.add(((q_base + p) as usize) as usize)) * *
                    (k.add(((k_base + p) as usize) as usize)); p = p + 1u32;
                } let raw = scale * dot; let causal_pass = if causal == 0u32
                { 1u32 } else { 0u32 }; let causal_le = if kj <= qi { 1u32 }
                else { 0u32 }; let causal_ok = if (causal_pass + causal_le) >
                0u32 { 1.0f32 } else { 0.0f32 }; let pad_ok = if kj < kv_len
                { 1.0f32 } else { 0.0f32 }; let valid = causal_ok * pad_ok;
                let sk = raw + (1.0f32 - valid) * (0.0f32 - 1000000000.0f32);
                let first = if kj == 0u32 { 1.0f32 } else { 0.0f32 }; let
                m_cmp = fmax(m, sk); let m_new = first * sk + (1.0f32 - first)
                * m_cmp; let a = (1.0f32 - first) *
                exp(fmin(m - m_new, 0.0f32)); let b =
                exp(fmin(sk - m_new, 0.0f32)); l = l * a + b; acc = acc * a +
                b * * (v.add(((kj * dv + vd) as usize) as usize)); m = m_new;
                kj = kj + 1u32;
            } if i < total { * (out.add((idx as usize) as usize)) = acc / l; }
            let in_bounds = if i < total { 1u32 } else { 0u32 }; let lane0 =
            if vd == 0u32 { 1u32 } else { 0u32 }; let write_stats = in_bounds
            * lane0; if write_stats == 1u32
            {
                * (stats.add(((2u32 * qi) as usize) as usize)) = m; *
                (stats.add(((2u32 * qi + 1u32) as usize) as usize)) = l;
            }
        }
    }
}
