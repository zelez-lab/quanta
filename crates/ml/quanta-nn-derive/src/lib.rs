...
fn expand(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let vars_doc = format!(
        " `{vars_name}` — the tape-bound mirror of [`{name}`], generated by \
         `#[derive(ParamTree)]`: every leaf is the corresponding parameter's \
         `Var`, in the same field layout."
    );
    let field_docs: Vec<String> = fnames
        .iter()
        .map(|f| format!(" Tape-bound mirror of `{name}::{f}`."))
        .collect();
    Ok(quote! {
        #[doc = #vars_doc]
        #vis struct #vars_name #impl_generics #where_tok {
            #( #[doc = #field_docs] #fvis #fnames: <#field_tys as #root::layer::ParamTree<#tp>>::Vars, )*
        }
