...
{
    let (h2_val, _s2) = alloc_handle(s1);
    assert(h2_val == s1.counter + 1);
    assert(h1 == s0.counter + 1);
    assert(s1.counter == h1);
}

/// T2502: alloc_handle never returns 0.
...
{
    // counter starts at 0, handle = counter + 1 >= 1.
}
