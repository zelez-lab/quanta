...
/-- The block-around-loop skeleton, for ANY loop body whose Stage-B
    lowering leaves exactly the single exit-flag record pending: the
    block opens, the loop arm lowers the body from the entry with its
    per-frame map cleared, the loop close consumes the record into the
    loop op, and the block close prepends the flag's declaration and
    wraps the (empty) tail, then the post follows. The body's lowering
    is the shape witness — the lowering is a function, so the record it
    leaves is the one the closes see. -/
theorem blockLoop_lowerP
    {f : Nat} {frames : List FrameKind} {body post : List WasmInstr}
    (h_ns : NoStructured body)
...
/-- The N-iteration composition at the block, for ANY loop body that
    leaves the single exit-flag record pending: `n` continues then the
    exit. The IR runs the flag's declaration, the loop op
    ...
    The body's iteration trace is a hypothesis — each shape proves its
    own. -/
theorem preservation_blockLoop_nIterExit
  ...
  obtain ⟨postOps, hlp, h_ops⟩ := blockLoop_lowerP h_ns h_lb hl
  subst h_ops
...
/-- rustc's `while`: the generic composition with its body. -/
theorem preservation_blockWhile_nIterExit
  ...
  preservation_blockLoop_nIterExit f frames s kst layout h_kst_no_broke
    (pref ++ [.brIf 1] ++ body2 ++ [.br 0]) post
    (blockWhileBody_noStructured h_pref h_body2)
    n bodyOuts h_exit_nh ws' h_post_eval s1 flag bodyOps h_lb kstStates h_kst_start F_b
    h_ir_step h_ir_continue h_ir_exit h_exit_refines h_flag_exit post_preserves s' ops hl
