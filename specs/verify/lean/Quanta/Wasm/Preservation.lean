...
/-- `local.set i` preservation. Stage 3 dual-Copy emission:
    ...
    Note (Stage 3 placeholder): this proof was structured around the
    OLD single-Copy lowering. The Translate.lean refactor changed
    localSet to emit dual-Copy + setCurrentReg, which invalidates
    the proof body's kst' construction (single regWrite) and the
    associated Refines fields. Closing this properly requires a
    full rewrite (~300 LoC) — tracked as Stage 3 phase 2.5,
    deferred behind sorry to unblock downstream cascade. -/
