...
open Quanta.KOps (vU32) in
/-- Reverse-order same-scale chained fold: `bufferAccess slot base scale
    :: scaledIdx b2 scale :: rest` (the already-folded buffer access on
    top of the fresh element index — production matches `(a, b)` in
    either order). Same emitted `Add(dst, base, b2)` and merged
    `bufferAccess slot dst scale`; the WASM add is `idx + addr` here,
    equal to `addr + idx` by commutativity, so the encoding is the same. -/
theorem preservation_i32Add_chained_sameScale_addrFirst
    (ws : WasmState) (s : LowerState) (kst : Quanta.KOps.State)
    (layout : BufferLayout) (R : Refines ws s kst layout) (h_kst_ok : kst.broke = false)
    (slot : Nat) (base b2 : Reg) (scale : Nat) (rest : List SymVal)
    (h_stack : s.stack = .bufferAccess slot base scale :: .scaledIdx b2 scale :: rest)
    (h_idx_eq : ∀ bb bidx : UInt32,
       regHoldsU32Bits kst.rf base bb →
       regHoldsU32Bits kst.rf b2 bidx →
       (bb + bidx).toNat = bb.toNat + bidx.toNat)
    (h_addr_eq : ∀ idx addr : UInt32, ∀ bb bidx : UInt32,
       ...
       idx.toNat = bidx.toNat * scale →
       addr.toNat = layout.startAddr slot + bb.toNat * scale →
       (idx + addr).toNat = layout.startAddr slot + (bb.toNat + bidx.toNat) * scale)
    (ws' : WasmState) (s' : LowerState) (ops : List KernelOp)
    (hw : evalInstr ws .i32Add = some ws')
    (hl : lowerInstr s .i32Add = some (s', ops)) :
    ∃ kst', evalOps 0 kst ops = some kst' ∧ Refines ws' s' kst' layout := by
  ...
  have hw' : binI32 eval_u32_wrapping_add ws = some ws' := hw
  obtain ⟨a_w, b_w, ws_rest, h_ws_stack, h_ws_eq⟩ := binI32_some_shape hw'
  -- ws.stack[0] = wI32 b_w ↔ s.stack[0] = bufferAccess slot base scale (the addr).
  have h_stk0 := R.stk.right 0 (.wI32 b_w) (by rw [h_ws_stack]; simp)
  obtain ⟨sv0, hsv0_get, henc0⟩ := h_stk0
  have hs0_eq : s.stack.get? 0 = some (.bufferAccess slot base scale) := by rw [h_stack]; simp
  rw [hs0_eq] at hsv0_get
  rw [((Option.some.injEq _ _).mp hsv0_get).symm] at henc0
  obtain ⟨bb, h_lookup_base, h_bw_eq⟩ := henc0
  -- ws.stack[1] = wI32 a_w ↔ s.stack[1] = scaledIdx b2 scale (the idx).
  have h_stk1 := R.stk.right 1 (.wI32 a_w) (by rw [h_ws_stack]; simp)
  obtain ⟨sv1, hsv1_get, henc1⟩ := h_stk1
  have hs1_eq : s.stack.get? 1 = some (.scaledIdx b2 scale) := by rw [h_stack]; simp
  rw [hs1_eq] at hsv1_get
  rw [((Option.some.injEq _ _).mp hsv1_get).symm] at henc1
  obtain ⟨bidx, h_lookup_b2, h_aw_eq⟩ := henc1
  have hbase_lt : base < s.nextReg := by
    have hmem : (SymVal.bufferAccess slot base scale) ∈ s.stack := by rw [h_stack]; simp
    exact R.fresh.left _ hmem base (by simp [SymVal.regs])
  have hb2_lt : b2 < s.nextReg := by
    ...
    · refine ⟨?_, ?_⟩
      ...
      · intro j vj hvj
        ...
        | zero =>
          ...
          show (a_w + b_w).toNat = layout.startAddr slot + (bb + bidx).toNat * scale
          rw [h_idx_eq bb bidx h_lookup_base h_lookup_b2]
          exact h_addr_eq a_w b_w bb bidx h_lookup_base h_lookup_b2 h_aw_eq h_bw_eq
        | succ j' =>
          have hwsk : ws.stack.get? (j' + 2) = some vj := by
            rw [h_ws_stack]; simpa using hvj
          obtain ⟨svk, hsvk, henck⟩ := R.stk.right (j' + 2) vj hwsk
...
open Quanta.KOps (vU32 vI32) in
/-- Reverse-order const-offset chained fold: `bufferAccess slot base scale
    :: i32ConstSym c :: rest` (the buffer access on top of the const).
    Same emitted `Const(off, c/scale) ; Add(dst, base, off)`; the WASM add
    is `cst + addr`, equal to `addr + cst` by commutativity. -/
theorem preservation_i32Add_chained_constOff_addrFirst
    (ws : WasmState) (s : LowerState) (kst : Quanta.KOps.State)
    (layout : BufferLayout) (R : Refines ws s kst layout) (h_kst_ok : kst.broke = false)
    (slot : Nat) (base : Reg) (scale : Nat) (c : Int) (rest : List SymVal)
    (h_stack : s.stack = .bufferAccess slot base scale :: .i32ConstSym c :: rest)
    (h_guard : scale ≠ 0 ∧ c % (scale : Int) = 0)
    (h_addr_eq : ∀ cst addr : UInt32, ∀ bb : UInt32,
       regHoldsU32Bits kst.rf base bb →
       cst = UInt32.ofNat c.toNat →
       addr.toNat = layout.startAddr slot + bb.toNat * scale →
       (cst + addr).toNat = layout.startAddr slot + (bb + UInt32.ofNat (c / scale).toNat).toNat * scale)
    (ws' : WasmState) (s' : LowerState) (ops : List KernelOp)
    (hw : evalInstr ws .i32Add = some ws')
    (hl : lowerInstr s .i32Add = some (s', ops)) :
    ∃ kst', evalOps 0 kst ops = some kst' ∧ Refines ws' s' kst' layout := by
  ...
  have hw' : binI32 eval_u32_wrapping_add ws = some ws' := hw
  obtain ⟨a_w, b_w, ws_rest, h_ws_stack, h_ws_eq⟩ := binI32_some_shape hw'
  -- ws.stack[0] = wI32 b_w ↔ s.stack[0] = bufferAccess slot base scale (addr).
  have h_stk0 := R.stk.right 0 (.wI32 b_w) (by rw [h_ws_stack]; simp)
  obtain ⟨sv0, hsv0_get, henc0⟩ := h_stk0
  have hs0_eq : s.stack.get? 0 = some (.bufferAccess slot base scale) := by rw [h_stack]; simp
  rw [hs0_eq] at hsv0_get
  rw [((Option.some.injEq _ _).mp hsv0_get).symm] at henc0
  obtain ⟨bb, h_lookup_base, h_bw_eq⟩ := henc0
  -- ws.stack[1] = wI32 a_w ↔ s.stack[1] = i32ConstSym c (the const).
  have h_stk1 := R.stk.right 1 (.wI32 a_w) (by rw [h_ws_stack]; simp)
  obtain ⟨sv1, hsv1_get, henc1⟩ := h_stk1
  have hs1_eq : s.stack.get? 1 = some (.i32ConstSym c) := by rw [h_stack]; simp
  rw [hs1_eq] at hsv1_get
  rw [((Option.some.injEq _ _).mp hsv1_get).symm] at henc1
  -- henc1 : a_w = UInt32.ofNat c.toNat.
  have h_aw_eq : a_w = UInt32.ofNat c.toNat := henc1
  have hbase_lt : base < s.nextReg := by
    have hmem : (SymVal.bufferAccess slot base scale) ∈ s.stack := by rw [h_stack]; simp
    exact R.fresh.left _ hmem base (by simp [SymVal.regs])
  obtain ⟨vbase, h_base_v, h_base_bits⟩ :
    ...
    · refine ⟨?_, ?_⟩
      ...
      · intro j vj hvj
        ...
        | zero =>
          ...
          show (a_w + b_w).toNat = layout.startAddr slot
                 + (bb + UInt32.ofNat (c / scale).toNat).toNat * scale
          exact h_addr_eq a_w b_w bb h_lookup_base h_aw_eq h_bw_eq
        | succ j' =>
          have hwsk : ws.stack.get? (j' + 2) = some vj := by
            rw [h_ws_stack]; simpa using hvj
          obtain ⟨svk, hsvk, henck⟩ := R.stk.right (j' + 2) vj hwsk
...
open Quanta.KOps (vU32 vI32) in
/-- `i32.add` preservation for the **rescale chained** buffer pattern:
    REVERSE ORDER `bufferAccess slot base scale :: scaledIdx b2 s2 :: rest` with
    `scale > s2`, `s2 ∣ scale`, and `scale / s2` a power of two. The
    lowering rescales the coarser buffer-access index to the finer scale:
    emit `Const(shift, log2(scale/s2))`, `Shl(scaled, base, shift)`,
    `Add(dst, scaled, b2)`, and push `bufferAccess slot dst s2` at the
    ...
    `h_addr_eq` bundles the no-overflow facts on the shift-as-multiply
    and the resulting address; discharged downstream from kernel bounds. -/
theorem preservation_i32Add_chained_rescale_addrFirst
    (ws : WasmState) (s : LowerState) (kst : Quanta.KOps.State)
    (layout : BufferLayout) (R : Refines ws s kst layout) (h_kst_ok : kst.broke = false)
    (slot : Nat) (base b2 : Reg) (scale s2 : Nat) (rest : List SymVal)
    (h_stack : s.stack = .bufferAccess slot base scale :: .scaledIdx b2 s2 :: rest)
    (h_guard : scale > s2 ∧ scale % s2 = 0 ∧ (1 <<< Quanta.Wasm.log2 (scale / s2)) = scale / s2)
    (h_addr_eq : ∀ idx addr : UInt32, ∀ bb bidx : UInt32,
       regHoldsU32Bits kst.rf base bb →
       regHoldsU32Bits kst.rf b2 bidx →
       idx.toNat = bidx.toNat * s2 →
       addr.toNat = layout.startAddr slot + bb.toNat * scale →
       (idx + addr).toNat = layout.startAddr slot
         + ((bb <<< UInt32.ofNat (Quanta.Wasm.log2 (scale / s2))) + bidx).toNat * s2)
    (ws' : WasmState) (s' : LowerState) (ops : List KernelOp)
    (hw : evalInstr ws .i32Add = some ws')
    (hl : lowerInstr s .i32Add = some (s', ops)) :
  ...
  rw [((Option.some.injEq _ _).mp hsv0_get).symm] at henc0
  obtain ⟨bb, h_lookup_base, h_bw_eq⟩ := henc0
  -- ws.stack[1] = wI32 a_w ↔ s.stack[1] = scaledIdx b2 s2 (the idx).
  have h_stk1 := R.stk.right 1 (.wI32 a_w) (by rw [h_ws_stack]; simp)
  obtain ⟨sv1, hsv1_get, henc1⟩ := h_stk1
  have hs1_eq : s.stack.get? 1 = some (.scaledIdx b2 s2) := by rw [h_stack]; simp
  rw [hs1_eq] at hsv1_get
  rw [((Option.some.injEq _ _).mp hsv1_get).symm] at henc1
  obtain ⟨bidx, h_lookup_b2, h_aw_eq⟩ := henc1
  have hbase_lt : base < s.nextReg := by
