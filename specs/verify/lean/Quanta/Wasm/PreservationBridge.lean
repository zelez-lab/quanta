...
/-- Dropping every per-frame binding keeps the refinement: reads fall
    back to the stable layer, which `LocalsRefines` already covers, and
    the three `currentReg` invariants are vacuous on `[]`. This is the
    frame-close step of the lowering's `wloop` / `wif` arms
    (production's `merge_locals_post_frame`) and the loop-entry step
    (`force_locals_to_stable`). -/
theorem Refines.clear_current
    {ws : WasmState} {s : LowerState} {kst : Quanta.KOps.State} {layout : BufferLayout}
    (R : Refines ws s kst layout) :
    Refines ws { s with currentReg := [] } kst layout :=
  ⟨R.stk, R.locs, R.fresh, R.aliasFree, R.injLocals, R.heapRefines,
      ...
      | some post_pair =>
          ...
          | some ws_after =>
              ...
              have h_lf_head : loopFreeNoBreak ops_head = true :=
                lowerInstr_localGet_emits_loopFreeNoBreak h_head
              ...
              -- ws_after = ws.push v: branchTarget / halted inherited.
      ...
      | some post_pair =>
          ...
          | some ws_after =>
              ...
              have h_mid_broke : kst_mid.broke = false :=
                evalOps_loopFreeNoBreak_preserves_broke
                  h_lf_head h_kst_no_broke h_kst_eval
              -- localSet pops + setLocal: branchTarget/halted unchanged from ws.
              have h_mid_no_branch : ws_after.branchTarget = none := by
...
theorem preservation_evalInstrs_cons_wif_trivialBodies
  ...
  | some pop_pair =>
    ...
    | some commit_triple =>
      ...
      rw [h_else_nil] at hl
      simp only [Option.some_bind] at hl
      -- The state passed to post: s_cast with the bindings cleared.
      cases hlp : lowerInstrs bt frames
          ({ s1 with nextReg := s1.nextReg + 1, currentReg := [] } : LowerState)
          post with
      | none => simp [hlp] at hl
      | some post_pair =>
        ...
        | some pop_w =>
          ...
          | wI32 c =>
            ...
            -- Apply post_preserves at the frame-close state.
            obtain ⟨kst', F_p, h_ev_p, R_p, h_bridge_p⟩ :=
              post_preserves R_at_cast.clear_current h_ws0_nb h_ws0_nh h_kst_cast_broke hw hlp
          ...
          | some pop_w =>
            ...
            | wI32 c =>
              ...
              · -- c = 0: WASM picks elseBody = []. ws_after_body = ws0.
                ...
                -- The state passed to post is s2 with its bindings cleared.
                have h_s2_restored_eq :
                    ({ s2 with localReg := s1.localReg,
                                localTy := s1.localTy,
                                currentReg := [] } : LowerState)
                    = { s2 with currentReg := [] } := by
                ...
                have R_at_s2 : Refines ws0 s2 kst_cast layout := by
                  ...
                  · -- LocalsRefines.
                    ...
                    have hfind_cast : ({ s1 with nextReg := s1.nextReg + 1 } : LowerState).localReg.find?
                      ...
                      rw [show ({ s1 with nextReg := s1.nextReg + 1 } : LowerState).localReg = s2.localReg
                          from h_s2_lr.symm]
                      exact hfind
                    rw [show (localTyOf s2.localTy i) = localTyOf s1.localTy i from by rw [h_s2_lt]]
                  ...
                  · -- CurrentRegRefines: s2.currentReg = s_cast.currentReg (from h_s2_cr).
                    show CurrentRegRefines layout ws0.locals s2.currentReg s2.localTy kst_cast.rf
                    rw [h_s2_cr, h_s2_lt]
                  ...
                  · -- CurrentLocalDisjoint: lift via h_s2_cr / h_s2_lr.
                    intro p q hp hq hpq
                    rw [h_s2_cr] at hp
                    rw [h_s2_lr] at hq
                    exact R_at_cast.curLocDisj p q hp hq hpq
                obtain ⟨kst', F_p, h_ev_p, R_p, h_bridge_p⟩ :=
                  post_preserves R_at_s2.clear_current h_ws0_nb h_ws0_nh h_kst_cast_broke hw hlp
              ...
              | wI32 c =>
                ...
                · -- c = 0: WASM picks elseBody, eval runs it.
                  ...
                  | some ws_ab =>
                    ...
                    have R_at_s2 : Refines ws0 s2 kst_cast layout := by
                      apply R_lift s2 h_s2_stack h_s2_lr h_s2_cr h_s2_lt h_s2_nr
                    obtain ⟨kst_ab, F_b, h_ev_b, R_b, h_bridge_b⟩ :=
                  ...
                  | some ws_ab =>
                    ...
                    have R_b_at_s3 : Refines ws_ab s3 kst_ab layout := by
                      ...
                      · intro i r hfind v hv
                        ...
                        rw [show (localTyOf s3.localTy i) = localTyOf s2.localTy i from by rw [h_s3_lt]]
                      ...
                      · -- currentReg: s3.currentReg = s2.currentReg (h_s3_cr)
                        show CurrentRegRefines layout ws_ab.locals s3.currentReg s3.localTy kst_ab.rf
                        rw [h_s3_cr, h_s3_lt]
                        exact R_b.currentReg
                      · intro r hr
                        rw [h_s3_cr] at hr
                        exact Nat.lt_of_lt_of_le (R_b.freshCurrent r hr) h_s3_nr
                      · -- curLocDisj: lift via h_s3_cr / h_s3_lr.
                        intro p q hp hq hpq
                        rw [h_s3_cr] at hp
                        rw [h_s3_lr] at hq
                        exact R_b.curLocDisj p q hp hq hpq
                    obtain ⟨kst', F_p, h_ev_p, R_p, h_bridge_p⟩ :=
                      post_preserves R_b_at_s3.clear_current h_ab_nb h_ab_nh h_ab_broke hw hlp
                    -- IR composition.
                    let F : Nat := max (max F_b F_p) 1
                    refine ⟨kst', F, ?_, ?_, h_bridge_p⟩
                    · rw [← h_ops_eq]
...
--   - Per-iteration body preservation evidence (each iteration's
--     transition matches across WASM/IR)
--   - Standard post and lowering IHs
--
-- The body is lowered from the loop-ENTRY state `{ s with currentReg
-- := [] }` and the post from the loop-CLOSE state `{ s1 with
-- currentReg := [] }` — the arm's binding discipline (production's
-- `force_locals_to_stable` / `merge_locals_post_frame`). No
-- structural hypothesis on the body's lowering is needed here: the
-- close step is hypothesis-free, and what the per-iteration evidence
-- needs of the body (its lowering returning to the entry frame) is
-- the caller's to establish (`PreservationWhile`).
-- ════════════════════════════════════════════════════════════════════

/-- The N-iteration theorem with the kernel's lowering already taken
    apart: the caller supplies `s' = s2` and `ops = loopOp bodyOps ::
    postOps`, which is how the Stage-B apex, whose post is not a plain
    lowering, reaches it. -/
theorem preservation_evalInstrs_cons_wloop_nIterExit_core
    (frames : List FrameKind)
    (ws : WasmState) (s : LowerState) (kst : Quanta.KOps.State)
    (layout : BufferLayout)
    ...
    -- The kernel's lowering, already taken apart (Stage A or Stage B).
    (h_s_eq : s2 = s') (h_ops_eq : [KernelOp.loopOp bodyOps] ++ postOps = ops) :
    ∃ (kst' : Quanta.KOps.State) (F : Nat),
      evalOps F kst ops = some kst' ∧
      Refines ws' s' kst' layout ∧
      BridgeClauses ws' kst' := by
  ...
  have R_reset : Refines (wasmBodyOuts (Fin.last n)) s1
    ...
    refine ⟨R_exit.stk, R_exit.locs, R_exit.fresh, R_exit.aliasFree,
            R_exit.injLocals, R_exit.heapRefines, R_exit.currentReg, R_exit.freshCurrent, R_exit.curLocDisj⟩
...
/-- `wloop _ :: rest` preservation, n-iteration exit case.
    ...
    per-iteration preservation evidence; the kernel's lowering is the
    plain Stage-A one, taken apart here. -/
