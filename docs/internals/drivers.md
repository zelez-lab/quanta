...
| Deferred submission (dispatches + render passes + resolves in one command buffer per sync interval) | ✅ one `VkCommandBuffer` per batch: compute run barriers, an ALL_COMMANDS barrier on each side of a pass/resolve, per-pass objects destroyed after the fence, every handle a recorded pass names pinned until submit | ✅ one `MTLCommandBuffer` per batch: lazily opened compute encoder, one render encoder per pass (encoder boundaries order tracked resources) | dispatches and passes submit one by one (no batch) — same semantics | dispatches execute inline; no render |
...
| GPU printf (`gpu_print_{u32,i32,f32}`) | ✅ record buffer at descriptor binding 30 (the JIT SPIR-V declares it; the driver allocates a TRANSFER field per printing wave, binds it on every dispatch path, drains to stderr after the fence and resets the cursor) | ✅ same scheme at `buffer(30)` (CPU-shared `MTLBuffer`; drained after the synchronous wait) | `NotSupported` (refused at validation — no WGSL scheme yet) | ✅ executor prints inline |
...
| Ray tracing | ⚠️ real BLAS path in `driver/vulkan/accel.rs` (AS create + scratch/storage alloc + command-buffer record), build short-circuited to `NotSupported` pending RT hardware (lavapipe segfault, awaiting AMDGPU runner) | ✅ **compute-based** (real `MTLAccelerationStructure` build — descriptor, sizes, encoder build, synchronous; indexed geometry; the ray-gen MSL compiles to a compute pipeline using `metal::raytracing::intersector`, dispatched with the AS at buffer(0) and the output at buffer(1); works on every Apple6+ GPU, no RT hardware — Apple RT silicon is M3+ and upgrades this silently). Separate hit/miss function tables are the native tier; `closest_hit`/`miss` are carried, not separately invoked | `NotSupported` | software lifecycle (dispatch recorded, output untouched) |
