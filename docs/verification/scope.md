...
(`specs/verify/lean/Quanta/Wasm/`) carries **407 theorems, 0
...
code interleaved with while loops whose straight-line bodies compute the
continue condition, any iteration count, loop state in locals or memory, under
the `LoopsTypeStable` label side condition; L10v7's
`framework_preservation_kernel` is a corollary. It is composed from
`framework_preservation_straightLine` and the N-iteration loop theorem
`preservation_evalInstrs_cons_wloop_nIterExit` (both `PreservationBridge.lean`).
Nested `block` / `wif` / `br` is outside it. Verus
...
- **Loop bodies that retag a local, and nested control flow.**
  ...
  are a `WhileBody` (a straight-line prefix of stack height one, closed by
  `brIf 0`), under the side condition `LoopsTypeStable`: no loop body changes
  a local's label (`localTy`) when lowered from the state the kernel reaches.
  The refinement relation is label-exact and the body is lowered once for
  every iteration, so a body that retags a loop-carried local (`u32 → i32` by
  adding a constant to a `u32`-labelled counter) is **outside the theorem** —
  production bridges such label crossings bit-exactly in the emitters.
  General nested `block` / `wif` / `br` is lowered and mechanized but
...
| 1 | Source Preservation | T590-T5B0 + Wasm corpus (407) | see [dashboard](index.md) | proven for the modeled subset (route a / step E) + the wasm route (step 059), each within its own boundary |
...
corpus (step 059) adds a further **407** Lean theorems, 0 sorries, in
