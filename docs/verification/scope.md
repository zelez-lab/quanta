...
(`specs/verify/lean/Quanta/Wasm/`) carries **475 theorems, 0
...
- **Loop bodies that retag a local, and nested control flow.**
  ...
  rebinds no local's stable register or label. The refinement relation is
...
| 1 | Source Preservation | T590-T5B0 + Wasm corpus (475) | see [dashboard](index.md) | proven for the modeled subset (route a / step E) + the wasm route (step 059), each within its own boundary |
...
corpus (step 059) adds a further **475** Lean theorems, 0 sorries, in
