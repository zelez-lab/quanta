...
| stream-ordered launches (async kernels, sync at the read) | the default: `gpu.dispatch()` encodes into one per-device batch, submitted at the next sync point (`flush()` / `pulse.wait()` / a `Field` read) — see [Execution model](../concepts/execution-model.md#deferred-submission) |
