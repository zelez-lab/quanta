...
| `nn.init.xavier_uniform_` / `kaiming_normal_` / … | `init::Init::{XavierUniform, KaimingNormal, …}.sample(&gpu, key, shape)` — build the params struct with the tensors you sampled | Initializers return fresh tensors instead of mutating; fans derive from the shape (`init::fans`), explicit-fan escape available |
| `safetensors.torch.save_file` / `load_file` | `safetensors::save(&params)` → bytes → your file IO; `safetensors::load(&gpu, &witness, &bytes)` | Same format on the wire — checkpoints cross between the stacks; loads are NAME-keyed with loud missing/extra/shape errors, and F16/BF16 files upconvert |
| `torch.cuda.amp.GradScaler` | `optim::LossScale` + `ScaleState` — `scaler.scale(...)` before backward, `scaler.unscale(grads, state)` after; `None` means skip the step | State passes explicitly (no hidden scaler object); power-of-two scales keep scaled runs bitwise equal to unscaled ones |
