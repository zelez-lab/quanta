...
— run it on the GPU with `cargo run --example mlp_training -p quanta-array
--release --features metal,autograd` (use `--features vulkan,autograd` off Apple, or
`--features autograd` alone for the portable CPU lane). (The example lives inside the crate, so it imports
`quanta_array::autograd::` directly; in your own app use `quanta::autograd`.)
