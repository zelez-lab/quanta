...
| `QUANTA_SKIP_METALLIB` | Set to `1` to skip `xcrun` metallib compilation. The Apple shader artifact becomes the MSL source (compiled by the driver at pipeline creation), exactly as on a host without the Apple toolchain. | Build time | For a Mac cross-compiling without the Metal toolchain in the loop. On macOS, a *missing* `xcrun` is otherwise a hard error (axiom A1: never silently drop the metallib). |
...
| `QUANTA_DEVICE` | Pick which discovered device `quanta::init()` returns on a multi-device host: a zero-based index into the `quanta::devices()` order, or a case-insensitive substring of the device name (`QUANTA_DEVICE=radeon`, `QUANTA_DEVICE=1`). Composes with `QUANTA_BACKEND`, which narrows the list first. No match fails `init()` with an error listing the devices that were found — it never falls through to a different device. Blank = unset. | Runtime | An integrated GPU plus a discrete card on the same backend (e.g. Iris Xe + an eGPU, both Vulkan): without this lever `init()` takes whichever the driver enumerates first. `quanta::devices()` is unaffected — it remains the programmatic selection path. |
| `QUANTA_VULKAN_LOADER` | The Vulkan loader library name opened at runtime — `vulkan-1.dll` via `LoadLibraryA` on Windows, `libvulkan.so.1` via `dlopen` on Linux, `libvulkan.so` on Android. Quanta never links the loader at build time on these platforms (the MoltenVK `vulkan-portability` opt-in on macOS is the one link-time arm), so no Vulkan SDK or `libvulkan-dev` is needed to build. | Runtime | Point it at an alternate loader, or at a nonexistent name to deterministically exercise the missing-loader path: a loud `quanta vulkan: … not found` line and the software fallback, with `QUANTA_BACKEND=vulkan` still refusing. |
