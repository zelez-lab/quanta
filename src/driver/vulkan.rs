...
impl GpuDevice for VulkanDevice {
    ...
    fn wave(&self, _kernel: &[u8]) -> Result<Wave, QuantaError> {
        // kernel bytes would be SPIR-V — create shader module + compute pipeline
        // TODO: implement when we have SPIR-V or LLVM output
        Err(QuantaError::CompilationFailed(
            "Vulkan compute not yet implemented — use MSL/Metal for now".into(),
        ))
    }

    fn wave_dispatch(&self, _wave: &Wave, _groups: [u32; 3]) -> Result<Pulse, QuantaError> {
        Err(QuantaError::SubmitFailed)
    }
