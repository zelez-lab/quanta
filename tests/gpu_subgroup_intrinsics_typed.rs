...
fn reduce_add_i32_returns_warp_sum() {
    ...
    assert!(result.iter().all(|&v| v == expected), "{result:?}");
}
...
fn reduce_max_f32_returns_warp_max() {
    ...
    for &v in &result {
        assert!((v - expected).abs() < 1e-3, "got {v}");
    }
}
...
fn scan_add_i32_runs() {
    ...
    // Inclusive scan of all-1s: result[lane] = lane + 1.
    for (lane, &v) in result.iter().enumerate() {
        assert_eq!(v, lane as i32 + 1, "lane {lane}: got {v}");
    }
}
